...
━━━ **实测：阈值 ＝ 20,480 字节，按字节，服务端自己说的（2026-08-28 二分实测）** ━━━
🔴 **服务端在拒绝时会明确回一条错误帧，把限额原样写在里面**：

    {"errcode": 40058, "errmsg": "body.markdown.content exceed max length 20480. "}

**二分实测逐条钉死（12 条探针，靶子全是 ShaoPeiShen 本人）**：

| 字节 | 字符 | 结果 |
|---|---|---|
| 20,431 | 6,833 | ✅ |
| 20,475 | 6,845 | ✅ |
| **20,480** | **6,850** | ✅ **最后一条发得出的** |
| **20,481** | **6,851** | 🔴 **多一个字节就发不出** |
| 20,520 | 6,862 | 🔴 |

⇒ **20,480 ＝ 20 KiB，边界是「> 20480 即拒」，本模块用 `<=` 判正好对齐。**

**按字节，不按字符——这一条由数据本身证死，不靠推断**：20,481 B 那条只有
**6,851 个字符**。若限额是「20,480 个字符」，它离限还差三分之二，不可能被拒。
它恰好在**第 20,481 个字节**上被拒 ⇒ 计的是字节。（旁证：队列 §四 `#47` 记着
群 webhook 也是「4096 **字节**」并且同样报 `errcode=40058`——同一族错误码、
同一种按字节的算法，只是两条通道的额度不同：群 webhook 4,096 B，智能机器人
20,480 B。）

🔴 **一处必须更正的既有诊断**：`波次收口-2026-08-26泳道版-2026-08-27.md` §十一
写着「企微服务端**对超限消息不回 ack**」——**不成立**。服务端**回了**，而且是在
约 60 毫秒内回的，内容就是上面那条写明限额的错误帧。**真因在 SDK 侧**：该错误帧
**没有 `headers.req_id`**，而 `aibot/ws.py::_handle_frame`（L235-276）按 `req_id`
路由——取到空串 ⇒ 既不在 `_pending_acks` 里、也不以 `SUBSCRIBE`／`HEARTBEAT`
开头 ⇒ 落到最后那句 `Received unknown frame` 只记一行 WARN，**pending ack 的
future 从此无人 resolve**，5.0 秒后超时。
⇒ **「调大 5.0 不是修复」这个结论仍然成立，但理由要换**：不是「服务端永远不答」，
而是「**答案 60 毫秒就到了，被丢掉了**」。⚠️ **代价最大的那一点**：这条写明限额的
WARN **每次失败都打在屏幕上**，包括 2026-08-27 `采购部#19` 连续失败那两次——
**答案一直在日志里，只是没人读那两行 WARN。**
...
#: 单条 markdown 消息的字节上限 ＝ **20,480**（20 KiB）。
#: 🔴 **这是实测值，不是估值、也不是凑的整数**：20,480 B 实测发得出、20,481 B
#: 实测发不出，服务端错误帧原文 `exceed max length 20480`（见模块 docstring 那张表）。
#: 判定用 `<=`，与服务端「> 20480 即拒」正好对齐 ⇒ **不留人为余量**：余量在这里
#: 只会白白多降级一批本来发得出的信，而边界本身已经是逐字节钉死的。
DEFAULT_MARKDOWN_MAX_BYTES = 20480
