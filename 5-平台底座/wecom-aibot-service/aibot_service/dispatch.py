...
KNOWN_RECIPIENT_USERIDS = {
    ...
    "泓钦": "Hongqin.Wang",
    "陈承": "2023458",
    # 李姣龙（财务部，2026-08-24 补，队列 #380）——唐燕萍 2026-08-22
    # 财务部#14 回件二次确认，与跟进信 README《企微 chatid 名录》独立互证
    # 一致。她同时是三件事的执行人（每工作日 10:00 前投放税务导出 Excel／
    # 每日核对群通知判流程健康／按《核对通过明细表》手动在 ERP 立账），
    # 我方已有多条动作需要直达她本人，不能再一律经唐燕萍转达。
    #
    # 🔴 **`2025672` 是纯数字工号，不是可推断值**（同 `陈承: 2023458`）——
    # 财务部六人里有两个是这种形态，任何"按拼音猜 userid"的做法在她们身上
    # 一定错，且错的形态是 fail-closed 静默跳过、命令行一切正常。
    #
    # 🔴 **本行是队列 #379（年度提醒）的硬前置**：收件人就是她，白名单不通
    # 则提醒发不出去，**且该通道 fail-closed 静默跳过、不报错**。
    "李姣龙": "2025672",
}
