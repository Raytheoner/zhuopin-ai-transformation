...
为六部门 35 个数字员工〔🔴 2026-08-21 v7 由 40 下调：质量域五场景下架〕提供**一套共享底座**，让每个场景只写业务逻辑，审计、OEM 隔离、ERP/SRM 接入、文档解析等"地基"复用同一份代码（单一可信源，满足 IATF）。读者：AIOps 开发与运维工程师。
...
5. **RAG/Chroma 落地**：per-OEM Collection 命名规范、嵌入模型、增量更新策略；~~Q1 客诉库结构化先行~~ 🔴 **2026-08-21 v7 更正：Q1 已下架，客诉库结构化不再是先行项**；~~Chroma 的现行消费方是 **Q4 PPAP 前置 ＋ QD-B 现行 OEM 红线**（v6 归属，且该归属须随 Q4 收敛重判一次，**重判出结论前 2027-01 硬截止不得放宽**）~~ 🔴 **2026-09-02 重判后现行口径（`#374` design 审，裁决人 Shao Peishen）**：**「Chroma 归属」拆为三层，本条只与 L3 有关** —— **L1 路由 guard（`OEMRouter`）✅ 已就绪、无到期日、不得下线**（QD-B 那条红线要的就是它，与 Chroma 建不建无关）／**L2 唯一入口 `rag.retrieve()` ❌ 未建**／**L3 Chroma 本体 ❌ 未接**。**Q4 与 Q2 都已不是 L2/L3 的消费方**（Q4 降存储侧身份；Q2 按 D1=(b) 采纯规则库判定、历史 8D 不进生产检索路径）⇒ **唯一有日期的消费方是 R1（2027-07，前置约 2027-05），L2/L3 到期日 ＝ 2027-05**，`2027-01` 已作废。🔴 **L3 本体这一轮到底还建不建，本身已是待判项 ＝ 队列 §一 `#465`** —— **本条 per-OEM Collection 命名规范等设计动作，在 `#465` 判定前不排期**。详见全景规划 §加速启动总览 Chroma 归属条。
