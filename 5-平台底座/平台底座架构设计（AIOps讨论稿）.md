...
5. **RAG/Chroma 落地**：per-OEM Collection 命名规范、嵌入模型、增量更新策略；~~Q1 客诉库结构化先行~~ 🔴 **2026-08-21 v7 更正：Q1 已下架，客诉库结构化不再是先行项**；Chroma 的现行消费方是 **Q4 PPAP 前置 ＋ QD-B 现行 OEM 红线**（v6 归属，且该归属须随 Q4 收敛重判一次，**重判出结论前 2027-01 硬截止不得放宽**）。
