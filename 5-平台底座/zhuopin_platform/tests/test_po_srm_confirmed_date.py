...
def _row(po_id: str, item_code: str, supplier: str, delivery: str) -> dict:
    return {
        ...
        "makeDate": "2026-06-01", "deliveryDate": delivery, "supplyCode": supplier,
    }
...
def test_no_srm_connector_behaves_as_before(tmp_path):
    ...
    rows = [_row("PO1", "M1", "V1", "2026-07-20")]
    with patch.object(zp, "_zp_post", return_value=rows):
        result = zp.get_purchase_orders()
    assert result[0].supplier_confirmed_date == result[0].expected_date == "2026-07-20"


def test_srm_confirmed_date_overrides_placeholder(tmp_path):
    """SRM 查到真实确认日期 → 覆盖占位值，且与 expected_date 不同。"""
    fake = _FakeSrmConn({"PO1": "2026-08-05"})
    zp = _make_zp(tmp_path, srm_connector=fake)
    rows = [_row("PO1", "M1", "V1", "2026-07-20")]
    with patch.object(zp, "_zp_post", return_value=rows):
        result = zp.get_purchase_orders()
    assert result[0].expected_date == "2026-07-20"
    assert result[0].supplier_confirmed_date == "2026-08-05"
...
def test_srm_no_match_falls_back_to_expected_date(tmp_path):
    ...
    with patch.object(zp, "_zp_post", return_value=rows):
        result = zp.get_purchase_orders()
    assert result[0].supplier_confirmed_date == "2026-07-20"
...
def test_multiple_pos_partial_srm_match(tmp_path):
    ...
    fake = _FakeSrmConn({"PO1": "2026-08-05"})
    zp = _make_zp(tmp_path, srm_connector=fake)
    rows = [_row("PO1", "M1", "V1", "2026-07-20"), _row("PO2", "M2", "V2", "2026-07-25")]
    with patch.object(zp, "_zp_post", return_value=rows):
        result = zp.get_purchase_orders()
    by_id = {r.po_id: r for r in result}
    assert by_id["PO1"].supplier_confirmed_date == "2026-08-05"
    assert by_id["PO2"].supplier_confirmed_date == "2026-07-25"  # 退回 expected_date
