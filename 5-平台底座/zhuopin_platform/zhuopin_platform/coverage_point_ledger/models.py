...
## ⚠️ 一处 schema 内部不一致（`OP-0906-Z` 如实登记，**本轮不替它做决定**）

SCHEMA §二 字段表写 `carrier` 「≥1（建点**或转态**行）」，但同节最小示例的第 2、3 行
（两条 `转态`）**都没有 `carrier`**。两者不可能同时成立。

本模块的取舍：**两类行都不在写入期强制 `carrier` ≥1**，理由两条，都是硬的：
...
🔴 **这不是裁决**：请在两包合审时与 SCHEMA §二 一并定死，届时改齐本处。
"""
...
class LedgerEvent:
    ...
    def __post_init__(self) -> None:
        ...
        if self.event is Event.CREATE:
            ...
            # ⚠️ `carrier` **刻意不在此处强制 ≥1** —— 见本文件末《一处 schema 内部不一致》。
