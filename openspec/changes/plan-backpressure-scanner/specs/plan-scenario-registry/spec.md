...
#### Scenario: 按域列取码，且沿革括注不参与
- **WHEN** 从权威排期表提取场景码
- **THEN** 各场景码前缀 SHALL 只在其对应的域列内生效（销售域沿革句「整域顺延入 S3」写在采购列时 MUST NOT 被计为采购域有 `S3`）
- **且** 单元格内的括注（`（…）`／`〔…〕`／`【…】`）SHALL 在提取前被剥除——其中装的是沿革引用（如「原 SC5 引擎承接部署位」），MUST NOT 被计为该表排了那个场景

#### Scenario: 表中未写码的场景由 registry 别名认出
- **WHEN** 某场景在权威排期表的对应域列中只以中文/缩写出现而未写场景码（实测：`Q2` 写作「8D①」、`Q6` 写作「立项门禁②」、`Q4` 写作「PPAP」）
- **THEN** 该场景在其域列命中 registry 的 `aliases` 时 SHALL 视为"表里有它"，MUST NOT 被报为「registry 有而排期表无」
- **且** 该兜底 SHALL 只使用 registry 中已声明的结构化别名，MUST NOT 解析单元格里的自然语言标题
