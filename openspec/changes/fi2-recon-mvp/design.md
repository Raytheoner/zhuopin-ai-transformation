...
- **CC 从紧解读（本次范围界定，待唐燕萍团队批改）**：本门禁**仅对"金额微差"分类生效**。理由：
  ...
  3. 若门禁面向"明细错位"生效，该类按定义（D11）本身要求 AP 级总额差异已在 `AP_LEVEL_AMOUNT_TOLERANCE`（0.5 元）内，几乎必然满足 R5 的 ¥1 门禁线，会导致"明细错位"永远自动降级 L2——但明细错位是**结构性问题**（料品间错位，涉及不同料品/成本归属，即便净额抵消也需人工核实是否记错了具体是哪个料品/项目），不应仅因总额小而免检。
  - **本项为 CC 依据定稿文字的收紧解读，非唐燕萍团队逐字拍板，需批改确认范围是否需要扩大**（例如"明细错位"是否也应纳入门禁——若确认纳入，需先解决上述"税额驱动的假阴性"风险，可能需要门禁改为"仅当该 AP 单下所有维度差异均可归因未税金额尾差"时才降级，而非现在的"classification==金额微差"简单判据）。
...
- **R5 门禁范围是否应扩大到"明细错位"**：见 D14-b 收紧解读，需唐燕萍团队确认。
...
- **外币供应商清单 + 跨运行历史状态设计**：见 D14-d，两者均是真实接入前必须补齐的前置，不在本次 mock 范围。
