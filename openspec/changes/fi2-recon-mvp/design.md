...
- **AP-PO 单价校验（D12/R7）意外发现**：10 个料品中 3 个超差（-6.1%~-44.7%），溯源发现一个**当前 design 未覆盖的真实缺口**——`price_check.py` 只比对 AP 单价与 `Purchase/Query` 的**原始下单价**，未消费 `POChange/Query` 的变更后价格；若 PO 有正式变更单调整过价格，比对基准未跟进更新，可能产生假阳性。三例中 1 例（PO 有 24 次变更记录）符合此假阳性风险，另 2 例（PO 零变更记录）排除此解释、判断为需业务侧核实的真实偏离。**未在本次 round-1 范围内改代码**（只读探测 `POChange/Query` 确认现象，未修 `price_check.py`），登记为独立后续设计评估项（tasks.md 14.13）。
  ...
  **⚠️ 后续评估已推翻本条假设（2026-07-28，队列 #80，tasks.md 14.14，CC 独立 worktree 真实探测，未改代码）**：Paul 已就"是否改用 PO 最新变更价"拍板方向 B（见 `1-转型规划/FI2-R7单价比对基准决策件-2026-07-23.md`），但正式实现前的真实只读探测发现 **`POChange/Query` 结构上无法支撑方向 B、且方向 B 的实现前提本身不成立**：
  1. **过滤参数名坑**：该端点过滤参数是 `PODocNo`（非其余财务端点统一的 `docNo`）；传 `docNo=<任意值>` **不报错**、静默返回全表（Total=2412，与传值无关），传对 `PODocNo` 才真正过滤（`PODocNo=ZPCG20220815002`→Total=24，与本节"24 次变更"吻合）——比此前 `AP/Query` 那类"列名无效直接报错"更危险（静默吐错数据，无报错信号）。
  2. **端点是 PO 单据级，非行项级**：返回字段仅 `PODocNo/ChangeDocNo/ChangeType/TaxRate/TaxAmt/NonTaxAmt/TotalAmt/ChangeFields`，无 `ItemCode`/`DocLineNo`/单价字段；`ChangeFields` 只是变更触碰过的字段中文名列表（如"最终价(本币)"），非具体新值；试 `itemCode`/`docLineNo`/`lineNo` 均不生效——结构上给不出"某料品最新单价"。
  3. **决定性交叉验证——`Purchase/Query` 现状已是最新价，不是"原始下单价"**：`ZPCG20220815002` 全部 25 行（`Purchase/Query` 实测）`TotalMnyTC` 求和＝23,875,291.60，与 `POChange/Query` 最后一条记录（`0POM2026030004`，2026-03）的"变更后"`TotalAmt`（同值）精确一致——证明该 PO 每次变更审批后主记录被 ERP **原地更新**，`price_check.py` 现用基准（`Purchase/Query.FinalPriceTC`）本来就是当前生效价，**不存在"基准过时"的 bug**。

  **结论：不接入 `POChange/Query`、不改 `price_check.py` 基准，方向 B 不实现**。推论：case①（`AP-2026040083`，−44.7%）大概率是真实价格偏差而非比对方法误判，与本节"八成不是真异常"的预判方向相反，需财务专线/唐燕萍业务侧核实。详见 `1-转型规划/session接力-财务域场景落地.md`【第十九轮】、队列 #80。
...
## D19：面板真实数据接入·轮1——u9c 模式 PO/AP 真实直读 + 发票人工誊录小样占位（2026-08-03，队列 #214/§四#43 拍板 (b)，🔴 待 Shao Peishen 审）

> 触发：唐燕萍 07-31 验收 v8 面板结构通过（队列 #191），唯一诉求"接真实数据再看一轮"——她的原话"现在跑的是 mock 数据，结构对不对能看出来，逻辑对不对看不出来"。§四#43 已拍板 (b)：本轮先接 PO+AP 两单真实数据、发票以人工誊录小样占位，并行跟催 OCR（#127）。范围与代价见 `1-转型规划/FI2真实数据接入时点-拍板素材一页-2026-08-03.md`。
>
> **与 D18（round-1）的关系**：D18（2026-07-23）已经用真实 U9C 数据跑通过一轮（6/8 样本、10 料品全匹配零假阳性），但走的是 `data_source=csv` + CLI（`python -m fi2.run --csv-dir data/real_round1`），产出的是 CC 侧一份验证报告——唐燕萍在 `.51:8094` 上点不到。**本轮（D19）不是重新验证逻辑，而是把 D18 已验证过的真实链路，接到她实际点开的那个 Flask 面板（`webapp.py` 的 `u9c` 模式）上**，且把面板 `u9c` 模式此前对发票"无条件 fail-loud"的行为，改为"若提供人工誊录小样则读取，否则维持 fail-loud"。

### D19-a：范围——五点，判定口径一律不动（§四#43 拍板原文，领取方勿自行扩缩）

1. 面板 `u9c` 模式接真实 PO（`Purchase/Query`，D15 已实现）＋ 真实 AP（`AP/Query`，D15/D16 已实现）——**这两项代码已就绪，本轮不改**，只是此前从未有人通过面板表单实际驱动到底（面板 `u9c` 模式此前每次都会在发票段 fail-loud，PO/AP 段的真实调用路径虽然代码存在但从未被端到端跑通过面板）。
2. 发票段从"如实报错"改为读一份人工誊录小样（8 组样本已在手，本节 D19-b 说明"round-1 誊录成果已确认无法复用、已重新誊录"的如实情况）。
3. 面板**显式标注**"⚠️ 发票为人工誊录小样，OCR 未接入"——沿用 v8 面板"不伪装已判定"的一贯红线（唐燕萍 07-31 认可"二期标灰徽标比做假按钮强"，同一逻辑）。
4. **判定口径本轮一律不动**：`match_engine.py`/`result_classify.py`/`price_check.py`/`config.py`/`models.py` 一字不改。本轮目的是让唐燕萍在真实偏差上表态，不是我方先改口径；她表态后才走口径变更（另立变更包）。
5. 交付后回请唐燕萍复核，**明确告知这是第一轮**，发票侧待 OCR（#82 round-2）到位后还有第二轮——她可能复核两轮是本方案已知代价，须提前讲明。

### D19-b：誊录成果处置——round-1 原件已确认无法复用，已重新真实取数+誊录（如实记录，非隐瞒）

D18（round-1，2026-07-23）誊录的 `data/real_round1/invoice.csv`（连同同目录 PO/AP CSV 快照 + PDF 原件）当时即按 `.gitignore` 的 `data/real_*` 规则**不入库**（红线：真实供应商单价/金额不进版本库，D18-b 已定）。**本轮开工前逐一核查**：仓库当前全部活跃 worktree + 主工作区，均**不存在** `data/real_round1/` 目录——round-1 的产出 worktree（`fi2-web-service-16da2a`）已在此后的台面清理中被删除，其未入库内容随之物理丢失。**结论：round-1 誊录成果确已无法复用，非"没找到位置"，是原件已不存在**。

**处置**：用真实 `ZpConnector` 等价 HTTP 调用（同一 `STOCK_API_BASE`/`STOCK_API_KEY` 凭据，只读 GET）重新拉取全部 8 组 AP 单的 `AP/Query`/`Purchase/Query`/`Attachment/List`/`Attachment/Download`——**结果与 D18-f 记录的 round-1 结论逐字复现**：6/8 可用（10 个料品，覆盖 6 个 AP 单）；AP-2025120181 复现同一 `Attachment/Download` 302 重定向到 `localhost:5555` 的服务端 bug（仍未修，非本轮引入）；AP-2026050057 复现为跨多 AP 单合并结算大票（100 行/8 页，发票金额>单张 AP 金额），仍超出人工誊录合理范围。**两组排除样本与 D18 完全一致，本轮范围维持 6 组、10 料品**，与 D18-f 记录的样本覆盖面相同。

**誊录方法**（逐张阅读发票 PDF 原文，人工对照 AP 侧配票记录反推料品编码，同 D18-a 方法论）：

- 5/6 张发票的料品编码可直接或按数量/金额精确交叉核对唯一确定（誊录行的 `untaxed_amount`/`tax_amount` 合计与对应 AP 明细行合计逐分精确一致，零人工估算误差）。
- **1 张例外，需记录清楚（AP-2026060004）**：该发票只开了 1 行（"集成电路 TLE4253E(4940)"，qty 10000，未税 23539.82），但对应 AP 侧实际是**两个不同内部料号**（`R01I.0085`/`J01I.0085`，物理上应是同一元件的两条不同产品线/BOM 版本领用），AP 两个料号的数量恰好相等（各 5000），发票未按我方内部码拆分。**誊录处置**：按 AP 侧数量占比对半拆成两条发票行（各 5000，未税各 11769.91，税额各 1530.09）。这是**人工假设**（对半拆分，理由是两个料号 AP 数量确实相等），不是发票原文携带的信息——由此产生的 AP↔发票金额比对会出现 ±0.01 元的分摊误差（AP 侧两料号原始合计分别为 11769.92/11769.90，与对半拆分后的 11769.91/11769.91 相差各 0.01 元），**如实保留，不做归零处理掩盖**；该笔差异远低于任何容差阈值，预期仍判"完全匹配"或被 R5 门禁自动消化，但结果以真实跑批为准，不预先断言。
- 誊录数据落 `data/real_round1/invoice.csv`（同 D18-b 目录名及 `.gitignore` 规则，PDF 原件落同目录 `attachments/`）——**本轮额外教训**：鉴于 round-1 的教训是"方法论细节只写在被 gitignore 的文件里、原始 worktree 一删就永久丢失"，**本次把上述誊录方法论摘要（尤其 AP-2026060004 的拆分理由）同时写入本 design.md（本段）与 `1-转型规划/FI2-round1真实验证报告-2026-08-03-面板轮1.md`（均入库、不受 worktree 生命周期影响）**，不再只留一份注释在 gitignored CSV 里。

### D19-c：实现范围——`feed_source.py`/`webapp.py` 两处，判据代码零改动

- `FeedSource.__init__` 新增可选参数 `invoice_sample_dir: Path | str | None = None`。
- `load_invoice()`：`data_source == "u9c"` 且 `invoice_sample_dir` 已提供 → 读取该目录 `invoice.csv`（复用既有 `parse_invoice`/`_InvoiceRow` 校验，与 mock/csv 源同一套边界校验逻辑，零新代码路径）；`invoice_sample_dir` 未提供（默认 `None`）→ 维持现状 fail-loud（`test_u9c_fail_loud_all_loaders` 等既有测试零回归）。`load_payment()` 不变（本场景不消费 payment，FI3 范畴）。
- `webapp.py`：`u9c` 模式表单新增一个**只读展示字段**（非用户可填的自由路径——避免用户在 Web 表单里填任意本机路径造成路径穿越/信息泄露面），后端固定指向场景内 `data/real_round1/`（与 D18-b 同一目录，已存在即用、不存在则该字段不生效、行为退回现状 fail-loud）。面板新增醒目标注：`u9c` 模式运行结果页数据源说明行里，发票列显式标注"⚠️ 发票为人工誊录小样，OCR 未接入"（复用 v8 已有的 `_STUB`/免责声明视觉语言，不新造样式体系）。
- **不改**：`match_engine.py`/`result_classify.py`/`price_check.py`/`recon_report.py`/`config.py`/`models.py`——本轮全部改动落在数据接入层（`feed_source.py`）与展示层（`webapp.py`），零判据代码改动。

### D19-d：判据零漂移验证方式

- 全量测试绿 + 零回归（新增用例覆盖：`invoice_sample_dir` 提供/不提供两条路径、mock/csv 源不受影响）。
- **mock 模式判定结果逐条比对**：改造前后对同一份 `data/mock/` 跑 `fi2.run.run()`，输出 `rep["items"]`/`rep["summary"]` 逐字段 diff，确认完全一致（本轮改动不触碰 mock/csv 路径的任何既有分支，理论上不可能漂移，仍做一次显式回归证明，不空口保证）。
- 真实数据端到端：面板 `u9c` 模式 + 6 组真实 AP 号 + `data/real_round1/invoice.csv` 跑一次，结果与 D18-f 记录的 round-1 结论（10 料品全"完全匹配"、3 例 AP-PO 单价超差）比对，一致则证明"接线正确、逻辑未变"；不一致则说明接线本身有 bug，需排查（不是判据变了，因为判据代码本轮未改）。

### D19-e：如实边界（不伪装已判定，FI2 一贯红线）

- 人工誊录小样**不是**规模化方案，8 组也不是"发票源已解决"——面板必须显式标注，避免唐燕萍误以为三单已全自动。
- AP-2026060004 的 50/50 拆分是本轮人工判断，不是发票原文自带的分类信息，已在 D19-b 如实记录，验收/复核时若唐燕萍或李姣龙对该笔有不同意见，以他们的业务判断为准（不是我方拆分方法本身有权威性）。
- 本轮只覆盖 6/8 样本，AP-2025120181（IT 附件下载 bug）与 AP-2026050057（合并结算大票）两组仍不可用，与 D18 状态一致，不因本轮"面板接线"而改变——这两项跟进方式不变（IT 跟进 / round-2 OCR 阶段优先覆盖）。
- 发票源真正的规模化路径仍是 OCR（队列 #59/#82/#127），本轮不改变该依赖关系，只是不再让唐燕萍等到 OCR 才能看到任何真实数据。

### Non-Goals（D19）

- OCR 自动直读集成——见 #82 round-2，与本轮无关、不冲突不重复。
- 判定口径/容差/五类判定优先级的任何调整——本轮明确不动，唐燕萍表态后才另立变更包处理。
- AP-2025120181/AP-2026050057 两个已知缺口的解决——维持 D18 既有跟进路径，不在本轮范围。
- 面板 Web 表单开放任意本机路径给用户自由填写发票样本目录——刻意收窄为固定路径展示型开关（见 D19-c 理由），避免引入路径穿越面。

### Open Questions（D19）

无——本轮范围已由 Shao Peishen 在 §四#43 拍板 (b) 时逐点写死，design 审核仅需确认"实现方式（D19-c）与如实边界（D19-e）是否可接受"，不涉及新的技术选型分歧。
