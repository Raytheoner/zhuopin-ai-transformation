...
> **🔴 附注（2026-08-22，OP-0821-J 僵尸未勾项清算；上方 2026-08-09 观察窗口声明原文一字未改）**：上方声明把 `1.5`／`10.13`／`11.12` 的文本当作当时的真实状态引用，据以判定“剩余未完成项的主体是等真实数据验证周期”——**这三项当时均已被事实超越**（1.5 已于 2026-07-10 交付、提前 7 周；真实数据验证已于 2026-07-23 round-1 与 2026-08-03 面板轮 1 完成）。本次逐项核实后共勾除 **7 项**（`1.5`／`1.6`／`10.13`／`11.12`／`14.13`／`15.10`／`15.11`），计数由 **116/129 → 123/129**，剩余 6 项经核实确属真未决（`7.3`／`7.4`／`8.3`／`8.4`／`10.11b`／`13.6`），逐项理由见各行与队列 §四 #91。⚠️ **上方声明的主判断（本包不属“疑似遗忘归档”）依然成立**，但其理由已换一条：不是“在等真实数据验证”，而是**在等 `10.11b` R5 分母口径唐燕萍批改 ＋ `7.3` 物料映射表基建 ＋ `8.3` 归档**。🔑 **上方声明自己已经看见了 15.10 那处字面矛盾并如实登记，却没有任何机制承接这条观察**——它被写成了散文，13 天后 `财务部#13` 照旧按 `1.5` 的字面去催了唐燕萍一次。**发现本身不是收口。**
...
- [x] 1.5 前置登记：唐燕萍 R1-R6 规则草案（约 2026-08 底）——交付后仅替换 config.py + 规则注册表，不改引擎 —— **✅ 2026-08-22 勾除（OP-0821-J，属漏勾、非新交付）**。**勾除依据**：规则草案已于 **2026-07-10 交付**（较本项所写的“约 2026-08 底”提前 7 周），载体＝《FI2-FI3-规则定稿-交CC-2026-07-10.md》（唐燕萍团队两份圈改回件 ＋ Paul 三拍板），见 `design.md` §D14 抬头（第 149-151 行）；落地完成于 commit `78927f1`（2026-07-16），`fi2/config.py` 中真值逐条在位可核：R1 `QTY_TOLERANCE_PCT`/`QTY_TOLERANCE_ABS`＝0（L19-20）、`AMOUNT_TAIL_TOLERANCE`＝0.5（L22）、`UNTAXED_AMOUNT_TOLERANCE_PCT`＝0.005（L25）、税额随税率（L26-28 ＋ `match_engine.py` L164）；R5 `L2_GATE_ABS_THRESHOLD`＝1.0（L36）；R7 `AP_PO_PRICE_TOLERANCE_PCT`＝0.02（L40）＋ `FOREIGN_CURRENCY_SUPPLIERS` 三家真值（L47-51）；`RULE_VERSION`＝`fi2-v3-tangyanping-2026-07-10`（L56）。R2 FIFO+人工指定／R3 五分类／R4 必填 9 字段／R6 孤儿单据标记，定稿件明写“维持已实现语义”、无需改动。🔴 **同一份交付在 7.1、9.14、§10 三处均已记 `[x]`，唯独本项漏勾**——`git log -S'1.5 前置登记' -- openspec/changes/fi2-recon-mvp/tasks.md` 只返回建档 commit `0d19918`，本行自 2026-07-07 起一字未改。
- [x] 1.6 前置登记：U9C 财务接口 / SRM 发票源 / OCR 选型（7/15 双反馈门）——真实数据验证前置，非 mock 开发阻断项 —— **✅ 2026-08-22 勾除（OP-0821-J，属漏勾）**。**勾除依据（三项前置已全部解除，解除方式各不相同，逐项列）**：① **U9C 财务接口 ✅ 已接入** —— design §D15，commit `471ced2`（2026-07-20）落 `ZpConnector.get_purchase_lines/get_gr_lines/get_ap_lines`；2026-07-21 apiKey 恢复后真实端到端跑通（本文件 11.2／11.7 已 `[x]`）。② **SRM 发票源 ✅ 整条出局** —— 财务域 v3（2026-07-09，全景规划 §0.2 该行）已定“SRM 900401 对财务域出局，FI2 发票走 U9C 应付附件”；v4（2026-08-04，唐燕萍回件：OCR 对货物名称及规格精确匹配仅 65.8%）再改道“税务系统导出发票明细 Excel”，摄取能力已建成并部署（队列 #295，openspec 已归档 `archive/2026-08-07-fi2-tax-export-ingest`；第 2 层定时扫描 ＋ 面板接入见队列 #82，2026-08-10 `.51` 真实验证通过）。③ **OCR 选型 ✅ 方案作废** —— 唐燕萍 2026-08-04 拍板停用，陈承同日独立回件口径一致，队列 #59／#127 已关闭。**“7/15 双反馈门”本身**已于 2026-07-09 判为“财务域数据闸实质解除”（全景规划 §0.2 2026-07-09 行）。⇒ 本项作为“前置是否解除”的登记项，三项均已终结，无一仍在阻塞。
...
- [x] 10.13 真实数据验证仍按 8 月底排期不变（规则提前定稿不等于提前上线，见规则定稿交接文件 §三纪律） —— **✅ 2026-08-22 勾除（OP-0821-J，⚠️ 属“被后续拍板整条推翻”而非“已交付”，如实标注）**。**勾除依据**：本项声明的“8 月底排期不变”已由 **Paul 2026-07-22 拍板「全力抢 8 月上旬」** 直接推翻（队列 #78 行内原文），其后真实数据验证实际发生于 **2026-07-23 round-1**（本文件 §14，6 组真实 AP／10 料品跑通，报告 `1-转型规划/FI2-round1真实验证报告-2026-07-23.md`）与 **2026-08-03 面板轮 1**（§15，`.51:8094` 真实端到端）。⇒ 该排期声明自 2026-07-22 起即不成立；继续挂 `[ ]` 只会被后来者误读为“真实验证尚未开始”——**2026-08-09 的观察窗口声明（本文件首行）即已如此误引**。
...
- [x] 11.12 真实小样本验证仍按 8 月底排期（本次只到“真实源代码可用”，不做批量真实对账跑批）；建议第一批次优先覆盖三家外币供应商 AP 单，补齐 D15-a③ 未定向核实的缺口——~~新增前置：需 apiKey 恢复~~ **✅ apiKey 已恢复**（2026-07-21），此前置已解除 —— **✅ 2026-08-22 勾除（OP-0821-J，属漏勾）**。**勾除依据**：本项要求的真实小样本验证已于 **2026-07-23 round-1** 完成（本文件 14.7-14.10，`FI2_DATA_SOURCE=csv python -m fi2.run --csv-dir data/real_round1` 跑通，6 AP／10 料品，零假阳性“明细错位”，3 例 AP-PO 单价超差已溯源）；本项特别建议的“第一批次优先覆盖三家外币供应商 AP 单”**已真实落实且可核**——验证报告 `1-转型规划/FI2-round1真实验证报告-2026-07-23.md` 第 22-31 行逐单列明：艾睿（AP-2026070036／070035）、上海英恒（AP-2026060004／040083）、安富利（AP-2026060073）**三家全部命中**，报告原文“6 组样本覆盖外币三家供应商（艾睿/上海英恒/安富利）全部命中”。D15-a③ 未定向核实的缺口随之补齐。
...
- [x] 14.13 OCR（腾讯云）自动直读集成——独立第二轮，不在本节范围，另行登记后续任务；同时登记一项后续：AP-2025120181 的 `Attachment/Download` 302 bug 需 IT（陈承）跟进 —— **✅ 2026-08-22 勾除（OP-0821-J，⚠️ 属“整条作废／已闭合”而非“已交付”，如实标注）**。**勾除依据（本项含两件，逐件列）**：① **OCR 自动直读 —— 方案已整条作废**：唐燕萍 2026-08-04 回件（8 张真实发票比对，货物名称及规格精确匹配仅 65.8%）拍板停用，陈承同日独立回件口径一致，队列 #59／#127 关闭；**round-2 已重定义为“税务导出 Excel 数据接入”并已建成部署**（队列 #295，openspec 已归档 `archive/2026-08-07-fi2-tax-export-ingest`；队列 #82 第 2 层定时扫描 ＋ 面板接入 2026-08-10 `.51` 真实验证通过）——本项所写“另行登记后续任务”这一动作本身已完成。② **AP-2025120181 的 302 bug —— 队列 #81 已闭合（2026-08-13，业务总线）**：陈承定性为**单例、非系统性**，并排除“存储路径写死 localhost”这一我方原猜测；我方答“暂不需要他加登录 cookie 兜底，附件下载随 OCR 方案一并作废”。⇒ 两件均已终结，无剩余动作。
...
- [x] 15.10 真实部署 `.51:8094` + 冒烟（`/api/ping`/首页/真实 POST `/run` u9c 模式端到端） —— **✅ 2026-08-22 勾除（OP-0821-J，属漏勾）**。**勾除依据**：部署与冒烟**实际发生在写下本项的同一个 commit `b04503f`（2026-08-03）里**——该 commit 的 message 明写“真实部署.51:8094冒烟通过”，其同批写入的场景 `CLAUDE.md`「部署状态 · 2026-08-03 面板真实数据接入·轮1上线」段记录**冒烟三件套全绿**：`/api/ping` 200；首页（带 `X-Auth-Token`）200；真实 POST `/run`（u9c 模式，6 组真实 AP 号 `AP-2026070036/070035/060004/040083/060073/070071`）200，报告页正确显示人工誊录标注，KPI 计数 5/2/3 与本机独立验证完全一致；`Fi2WebServer` 新进程存活确认（`NEW_PID=1424`）。**本文件 15.12 的注文亦自陈**“发送前已核对…发送硬前置三条件（代码入 master／部署冒烟／真实案例复现，均已满足）”。⚠️ **本项系依据上述书面留痕勾除，本次未重新登录 `.51` 复冒烟**（派单件 OP-0821-J §四明写“不碰 `.51`”）。
- [x] 15.11 场景 `CLAUDE.md` 更新状态段；跨桌任务队列 #214 回填结果（本节完工不代表整个变更包 `fi2-recon-mvp` 可归档——归档判断以变更包全部任务而非单节为准，本次不执行 `/opsx:archive`） —— **✅ 2026-08-22 勾除（OP-0821-J，属漏勾）**。**勾除依据（两件均可核）**：① **场景 `CLAUDE.md` 状态段已更新** —— 同一 commit `b04503f`（2026-08-03）新增「部署状态」段之「2026-08-03 面板真实数据接入·轮1上线」小节（该文件 44 行改动）；② **跨桌任务队列 #214 已回填** —— 该行现存于 `1-转型规划/0-全景路线图/跨桌任务队列-归档-202608.md`，状态列 **✅ 已交付（2026-08-03，CC）**，并如实登记了 worktree 实名与派单件写死名称不一致一事。⚠️ 原括注举的例子“14.13 OCR round-2 等既有开放项仍在”**已随本次 14.13 勾除而失效**，故改写为不举具体例子的等义表述；**“本次不执行 `/opsx:archive`”的结论不变**——8.3 仍 `[ ]`（见下）。
