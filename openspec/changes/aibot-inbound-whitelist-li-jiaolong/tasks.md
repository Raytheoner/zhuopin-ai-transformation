...
> **状态：design 审已全批（Shao Peishen 2026-08-25，队列 §四 `#116`「全按建议」），apply 已完成。**
> 落地环境：CC 无头批处理 A30，分支 `claude/a30-inbound-whitelist-apply`，worktree `.claude/worktrees/a30-inbound-whitelist-apply`。
> **§4「真实发送冒烟」整节留步未做**——对外动作，无人值守不发，判据原样保留、不得放宽。
>
> 🔴 **本包本次不归档，理由写在机器认得的地方（根 `CLAUDE.md`「完工即归档纪律」三入口之一）：`预期观察窗口：7 天`。**
> 判据取值理由：§4 的三条与 5.3 全部卡在「须有人在场做一次对外动作」，`#379` 年度提醒 **09-01 首触发**，故观察窗口取到那一天（2026-08-25 起 7 天）。**不是「作者对未来的永久声明」故不用 `暂不归档`**——这件事有明确的到期日，超窗就该升级，而不是永久静默。
...
- [ ] 4.3 一并验回件方向：她回一句话后不再触发 `whitelist_rejected`，而是正常归档

> **留步原因**：向真人发企微消息属对外动作，本 session 为无人值守批处理，按纪律不发。**A20 重启后出站配置已确认生效于运行码**（见 `proposal.md` 复核表），故本项一旦有人在场即可直接执行，无技术前置。
> 🔴 **本包 apply 的代码同样尚未生效于运行码**：运行中的监听器进程跑的是 worktree `wecom-service-home`，本包提交在 `claude/a30-inbound-whitelist-apply` 分支上、**未合 master、未重启监听器** ⇒ **此刻李姣龙若回话，仍会被挡回**。生效路径＝ ff 合入 master → 该 worktree 同步 → 重启监听器，属对外/运行态动作，同样留人在场（见队列 `#380` 回写）。

## 5. 收口

- [x] 5.1 回写队列 `#380`（propose 段 A21 已回写；本次 apply 段再回写一次）
