> **预期观察窗口：7 天**（队列 #314②，2026-08-13 声明，基准日 2026-08-13，预期 2026-08-20 满周）——2026-08-09 判定的"待定"（见下方存档）已解除：7.2/9.1-9.4 已于 2026-08-13 补齐（详见各任务行），本包**现已仅剩 8.5 一项被动观察项**（10.5 是明确的二期延后项，不计入本包当前范围，见该行）。8.5 的观察时钟起算口径由 Shao Peishen 2026-08-13 当场拍板：**本服务永不部署 `.51`**（开发期专员协同工具，系统上线后不再需要；后续开发迁往 Mac Studio，届时在 Mac 重建一套），故不必等 `.51` 正式发布，**满周判定即日起在当前开发环境起算**，对应队列 §四 #62 已选 (a)。**⇒ 状态去匹配告警，而非改告警去匹配状态**（Shao Peishen 原话）：待 8.5 满周验收完成，本包才真正只剩历史记录，届时按纪律走 `/opsx:archive`。
>
> ━━━ 存档（2026-08-09 原判定，历史记录不追改）━━━ **预期观察窗口：待定**（队列 #314②，2026-08-09 判定，未声明数值窗口）——如实登记：本包剩余未完成项**不是纯被动观察项**，故不套用观察窗口机制，维持现状（正常"疑似遗忘归档"告警流程）。判据：7.2（人为断网重连自愈验证）与 9.1-9.4（场景 `CLAUDE.md` 撰写／队列 #18 回写／文档台账重跑／commit）均是当前即可主动完成的具体工作，非等待外部触发；8.5（真实群运行满一周）虽属观察型，但其观察时钟尚未起算——阻塞点是"在开发环境跑满周还是等 `.51` 正式发布后再计"这一决策，需 Paul 定，与"已在观察、等窗口到期"性质不同；10.5（二期 dry-run 观察）已明写"本次不做，登记后续待领行"，不计入本包当前范围。**⇒ 若要让本包受益于观察窗口机制，须先完成 7.2/9.1-9.4 或补齐 8.5 的前置决策，届时可重新声明窗口**——本次不代为判断或代为拍板，如实登记交后续处理。
...
- [x] 7.2 验证断网重连自愈（人为断网/杀进程观察退避与恢复）——**2026-08-13 完成**（开发环境真实杀进程测试，Shao Peishen 拍板"不必等 `.51`"当场执行）：`Stop-Process -Force` 杀掉当时活跃的 `run_aibot_service.py`（PID 11448，2026-08-13T03:16:44Z）；`start-aibot-service-dev.ps1` 外层三级退避包装如设计检出退出、稳定运行 74607 秒后档位归零、走第 1 级 60 秒退避（`reports/service-dev-20260812.log` 留痕：`[11:16:44] 服务退出...第 1 级退避：等待 60 秒后重启...[11:17:44] 启动...第 2 次`）；60 秒后新实例（PID 33640）自动拉起，`reports/wecom_aibot_audit.jsonl`（主工作区路径）记录 `connection_established`（03:17:48.127Z）→`authenticated`（03:17:48.398Z）→`gap_alert_sent`（03:17:49.625Z，"每次(重)连接都发通报"机制，收件人 ShaoPeiShen）完整链路，`reports/aibot_liveness.json` 存活戳同步更新为 `03:17:50.723Z`。**全链路自愈时间约 66 秒（60 秒退避 + 约 6 秒重连认证），零人工干预**。
...
- [x] 8.5 真实群运行满一周，观察无误归档/无漏推送/audit 全程可查，产出验收小结供 Paul 签字批准转常驻（对应 proposal.md 晋档条件）——**观察中，未完成**：Shao Peishen 2026-08-13 拍板解除阻塞——本服务**永不部署 `.51`**（开发期专员协同工具，系统上线后不用；后续迁 Mac Studio 重建），故观察时钟**即日起在当前开发环境起算**（对应队列 §四 #62 选 (a)），基准日 2026-08-13，预期 2026-08-20 满周。满周后需回本行核实观察期内确无误归档/无漏推送、`audit` 全程可查，产出验收小结后才可勾选，**不得未观察先勾**。
  - ⏳ **验收小结已出、待签字（2026-09-12，CC 无头泳道 `OP-0912-M`，分支 `claude/op0912m-wecom-85-acceptance`）**：`1-转型规划/0-全景路线图/验收小结-wecom-aibot-channel-2026-09-12.md`——观察期预期 7 天／实际 30 天 18 小时（非连续，最长连续 2.49 天）；无误归档 55/55 ✅、无漏推送 28 应推／27 已发＋1 已批准待人工发 ✅、audit 事件链可追 ✅ 但 hash-chain 窗口内 269 处并发分叉断点（`#564` 修复未上线）⚠️；本行**仍不勾**——满周口径（累计 vs 连续）与转常驻前置由 Shao Peishen 在小结 §五 签字区裁（Q1–Q4）。

## 9. 收尾

  - 🟩 **8.5 签字（Shao Peishen 2026-09-12 回 `3:Q1a,Q2a,Q3a,Q4a`，经 Cowork 环境总线 `OP-0912-E` 转达；小结＝`OP-0912-M` 泳道产出）**：
    **Q1 ＝ (a)**：观察跨度按**累计**计，实测 30 天 18 小时即算满足。🔴 **如实记边界**：该 30.8 天**非连续**——>5 分钟断线 14 段共 19.8 h（2.69%），进程启动 52 次，最长连续段 2.49 天；泳道结论「**连续 7 天在笔记本形态下不可能达成**」，故改按累计口径签认，**不是**声称已连续满周。
    **Q2 ＝ (a)**：`verify_chain()` 的 340 断点（窗口 269）**只标注、不回填**，转常驻**不另设前置**。判据：269/269 可归因并发分叉、**0 处疑篡改**；且 `#564` 的修复 `59b54f7` 已于 2026-09-12 08:42 进 master、常驻进程 19:02 已重启并验活 ⇒ **断点全属修复上线前的历史数据**。（⚠️ 小结原文写的「已 push、未 ff、运行进程不含」经 `OP-0912-E` 核否，以本段为准。）
    **Q3 ＝ (a)**：`#545` 附件超时修复（分支 `claude/op0910j-sdk-request-timeout-545`，`merge-tree` 零冲突）并入同一次 ff ＋ 重启。
    **Q4 ＝ (a)**：**10.5 二期转出为独立待领行**，不在本包内做 ⇒ 9.5 归档解锁。
    🔴 **仍未覆盖的结构性盲区（不随签字消失）**：aibot 协议无离线补推，离线期间专员消息永久丢失、本方无法证明其不存在。

- [x] 9.1 `wecom-aibot-service/CLAUDE.md`（六段式：定位/决策/底座/红线/时间线/依赖）——**核对后补勾（2026-08-13）**：该文件此前已存在于磁盘且持续维护至 2026-07-31（六段齐全，§5「状态时间线」记录 20 余轮真实修复/部署，远超原任务字面要求），仅此 tasks.md 检查框未同步——本次核对内容确实符合六段式要求后补勾，并同批追加 2026-08-13 状态时间线新条目（7.2/8.5/9.1-9.4 收口）
- [x] 9.2 `跨桌任务队列.md` #18 状态回写（本 session 完成部分：B/C/D 段代码+单测，非"完成"——6/7/8 段待部署脚本+真实凭据）——**核对后补勾（2026-08-13）**：队列 #18 早已随后续多轮真实进展持续回写、并已归档至 `跨桌任务队列-归档-202607.md`（完整记录场景①②真实验证、四个生产 bug 修复、真实群灰度、常驻监听等全过程），仅此 tasks.md 检查框未同步——核对归档内容确实完整反映真实进展后补勾
- [x] 9.3 收工重跑文档台账（`0-学习与工具/工具-文档台账生成.py`）——2026-08-13 已重跑，随本批次一并 commit
- [x] 9.4 全部新产出/修改文件 commit（本 worktree 分支，是否 push 待 Paul 确认）——2026-08-13 CC 独立 worktree `env-alert-wording-a批` 已 commit + push master（本次仅涉 openspec 变更包文档，未改场景代码）
- [ ] 9.5 `/opsx:archive wecom-aibot-channel -y`——**仍不执行**：2026-08-13 7.2/9.1-9.4 已补齐，但 8.5（观察窗口进行中，预期 2026-08-20 满周）与 10.5（二期延后，明确不在本次范围）仍为 `[ ]`，未达"全部 [x]"，"完工即归档"纪律的前置条件尚不满足；8.5 满周验收完成后再评估是否归档（10.5 是否需先转移为独立待领行、还是允许挂账不阻档，留待归档评估时一并决定）
...
- [x] 10.5 二期（待观察，不在本次范围）：dry-run 观察一段时间（建议 1-2 周）确认误报率可接受后，评估是否开启自动补行——登记跨桌任务队列待领行，本次不做
  - 🟩 **2026-09-12 按 Q4(a) 转出**：本项（dry-run 观察后评估是否开启自动补行）**不在本包内做**，已转为独立待领行；本包据此解锁 9.5 归档。
