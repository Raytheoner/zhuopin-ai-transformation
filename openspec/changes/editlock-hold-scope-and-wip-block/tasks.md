...
- [🟡] 4.4 **真实 dogfooding —— 只取到一半样本，如实记录不夸大**：本项原预期「apply 自身的 release 会撞上 ⑨」，**实际没有撞上，而这正是设计对的表现**——本次 release 只改既有行、**未新增任何 `[D:机]` §一 行**，按决策点 4 的触发条件根本不进入判定（实测当刻可动 WIP ＝ **23／16，确已超限**，release 仍 exit 0）。⇒ **本次真实取得的是决策点 4 的关键回归样本**（超限 ＋ 未新增机制行 ⇒ 照常放行，规则没把来关行的 session 锁在门外），**不是**晋档条件①要的「真的被拒绝」样本。**晋档条件①②仍无真实样本**，须等下一个真正新立机制行的 session 自然发生，届时按 §四 #58 回填。
- [x] 4.5 队列 #324（写 🟡 不写 ✅，子项②③④⑵⑤未处理）与 §四 #58 ⑶ 均已回填；协议〇.9 措施 C 正文同批改写；§二 批次 `B-0817_队列324与58之3_编辑锁两处apply回写` 已由 sweep 落库（commit `a189e56`）。**两次 release 均走新版工具**（第二次是为更正下一条那个被自己证伪的监测口径）。
- [x] 4.6 `专线opener模板库.md` §〇.9 ⑵ 已改写：写明超限时 `release` 会真的拒绝、触发条件仅限「真正新增 `[D:机]` §一 行」、两条出路的确切写法、以及「理由必须写进行里」的理由。**顺带更正一处存量陈旧值**：该节标题写的上限仍是 **8**，而 2026-08-09 已拍板上调为 **16**（协议〇.9 正文／`MECHANISM_WIP_CAP_DEFAULT`／看板三处早已是 16，本库是**第四处此前无人知道它也载有该数值**的地方，与 2026-08-17 巡检在 skill 里发现的同款遗漏同源）。
- [ ] 4.7 `/opsx:archive editlock-hold-scope-and-wip-block -y`（全部 tasks 勾完才做）—— 🔴 **本次不做，前置条件未满足，如实登记不假装完工**：4.4 只取到一半样本（晋档条件①②仍无真实样本），本项须等 ⑨ 真实拒绝过一次、逃生阀真实用过一次之后再评估；届时若那两次仍未自然发生，须先回 §四 #58 讨论"要不要人为构造一次"，而不是直接归档。
