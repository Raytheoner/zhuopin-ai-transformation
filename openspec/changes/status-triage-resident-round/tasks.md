...
- [x] 7.3 队列 `#454` 已回写为 **`[S:partial]`**（不写 `[S:done]`：6.3 须待合入 master 后的第一轮 sweep 才能取证，本行已点名该唯一留步项）；§二 批次 `B-0906_N_OP0906N_状态分诊常态化apply回写` 已登记为「待处理（登记）」，锁已 release（`status` 反查 ⇒「无锁」）。🔴 **回写时撞上 K2 行长上限、就地按口径处置**：把新段直接追加会让整格达 6,225 字符（约 9.4 KB，远超 `ROW_LENGTH_CAP_BYTES = 4096`，2026-09-11 起由告警转阻断）——按 K2「行内只留首段＋指针＋末段」，将行内「本次操作撞出的机制缺陷」段与「`OP-0906-G` design」段**原文原样**外置进 `1-转型规划/0-全景路线图/队列行日志/#454.md`（第二批，2 段，md5:`e679710b`），apply 收工全文同入该件，行内末段只留结论与留步项 ⇒ **整格 3,757 B，合规**。
- [ ] 7.4 （前置 6.3）`/opsx:archive status-triage-resident-round -y` —— **待 6.3 留痕后**再归档：归档一条尚未验证过出口的告警类变更，等于把"它到底响没响"这个问题永久埋进 archive。（🟩 2026-09-12 `OP-0912-AA`：原写在 §7 标题上的前置声明（指向 6.3）下移到本项——7.1–7.3 不依赖 6.3、已按实做完，标题级声明过宽致 `工具-僵尸未勾项lint.py` J-A 误命中 3 条；只移声明位置，不改任何勾选。）
