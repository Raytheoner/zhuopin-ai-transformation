...
- [ ] 8.5 真实群运行满一周，观察无误归档/无漏推送/audit 全程可查，产出验收小结供 Paul 签字批准转常驻（对应 proposal.md 晋档条件）——**观察中，未完成**：Shao Peishen 2026-08-13 拍板解除阻塞——本服务**永不部署 `.51`**（开发期专员协同工具，系统上线后不用；后续迁 Mac Studio 重建），故观察时钟**即日起在当前开发环境起算**（对应队列 §四 #62 选 (a)），基准日 2026-08-13，预期 2026-08-20 满周。满周后需回本行核实观察期内确无误归档/无漏推送、`audit` 全程可查，产出验收小结后才可勾选，**不得未观察先勾**。
