...
- [ ] 2.2 🔴 **断言测试：封存不改一个字节**（D3）—— 封存前后对原热文件内容做 sha256 逐字节比对，段文件必须全等；且断言实现里**没有任何重算 `prev_hash` 的代码路径**【CC】
  - 成因：**2026-07-28 本项目已重算过一次链，且 `verify_chain()` 通过**（CHANGELOG 第 43 行）。⇒ 重算过的链验证永远是 ✅，哈希链对整理动作零防御力。
  - ⚠️ **那次重算是已批准的正当修法，不是事故**（真实缺陷是审计流分裂成两份、闭环无法在单一文件内还原，队列归档 #126 行；原件亦另存未删）。**本条援引它作存在性证明**：动机完全正当的一次整理就足以无痕重写整条链而校验照过 ⇒ **靠"别去重算"这种纪律拦不住，必须在机制上堵死。**
