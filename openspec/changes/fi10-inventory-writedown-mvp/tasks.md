...
> ✅ **2026-09-06 design 审（`OP-0906-V`）关闭 2.1／2.4／2.6**；**2.3 仍开**（只阻断 §4／§5，不阻断 §3）。**2.2 已于 2026-09-17 由业务总线追加关闭**（`design.md` D14，见下；当时 design 审确认「不阻断 §3」的结论不变，只阻断本轮本就不实现的芯片降价预警）。
...
- [x] 2.2 收口-2（**须先于选型**）：「芯片供货 API」与「芯片市场价格 API」是否同一项——**判定前不得开始选型，否则可能选错标的**（`#475`）。属规划文档口径 ⇒ 全景路线图线处置，**本泳道不代判**
  ⇒ ✅ **已由业务总线 2026-09-17 裁定（队列 `#609`，承 `OP-0911-H`）：非同一项，是两项被一个条目混着写了**——详见 `design.md` **D14**。三处载体逐字取证：前置总表 §一 O2 行「芯片**供货** API」（供货侧：LT 变更／停产／供应风险）；实施计划 §外部依赖表「芯片**市场价格** API｜O2 芯片预警子功能（原 SC6）／FI10」（价格侧：市场价格波动）；全景规划 §12月交付物段只写「各依赖外部 API」、未说明是同一个。🔑 **O2 要「供不供得上」，FI10 要「现在值多少钱」——两个数据产品、两份合同、两套字段**；实施计划并成一行是 2026-07 写表时的简写，不是判定结论。
  🔴 **本判定只解锁 FI10 价格侧标的描述，不解锁任何采购动作**——选型属采购／IT，O2 侧供货 API 选型另有排期，**本泳道不起选型、不联络供应商**。**不阻断 §3**（§3 已完成，见下）：本条原只阻断芯片降价超阈值预警，而该预警本轮本就不实现（`5.1`／`6.4` 不变，仍待 `#475` 前置满足后另起变更包补做）。
