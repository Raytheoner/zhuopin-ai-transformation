...
- [ ] 2.4 收口-4：发票号字面一致性实测（位数／前导零／空格／全半角／代码前缀）→ 定 join 键。**不得沿用 FI2 后 8 位方案**。🔴 **状态勿在此复述**（会过时）：探针已就位（`scripts/probe_606_invoice_no_literal_consistency.py`），实测结论与卡因见 `design.md` 2026-09-17 追记
- [ ] 2.5 收口-5：报销发票 OCR 精度实测（不沿用 FI2 65.8%，也不沿用「与本次无关」直接判可用）。🔴 **状态勿在此复述**：实测尝试与两项前置缺口见 `design.md` 2026-09-17 追记
