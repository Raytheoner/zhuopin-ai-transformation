...
> 🟩 **已追认 (a)（Shao Peishen 2026-09-12 回 `2:1a2a`，经 Cowork 环境总线 `OP-0912-E` 转达）**：取「两列任一以 🛑 起首即排除」，与 `_count_mechanism_wip` 的 `OP-0911-N`／design D-B 甲口径同尺。`#382`／`#448` 不回池。实现已在 `open_pool.judge_open_pool_row` 第 4 条，无需改动。
...
> 🟩 **已追认 (a)（Shao Peishen 2026-09-12 回 `2:1a2a`，经 Cowork 环境总线 `OP-0912-E` 转达）**：缺 `[D:]` 的可动行归 degraded 单列、不入池，推送器发 `RuntimeWarning`；与 `#523`「缺域行不猜域、只告警」同口径。当前生产零即时影响，实现无需改动。
