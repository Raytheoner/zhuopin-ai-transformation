...
- [ ] 1.6b **A4 备份堆积**（2026-08-30 并入，rebase 后发现）：数同目录 `CLAUDE.md*.bak*` 个数与最旧日期，超 `GLOBAL_MEMORY_BAK_CAP`（默认 3）告警；🔴 **只报不删**；与 A1-A3 共用同一次目录读取
...
- [ ] 2.3b A4 单测：5 个 .bak 命中告警并列最旧件；≤3 个只回显不告警
