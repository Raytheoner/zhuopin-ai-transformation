...
- [x] 6.3 队列 §一 `#504` 状态回写 ＋ §二 批次登记
  - **2026-09-09 `OP-0909-M2` 续棒完成。** §一 `#504` 状态格整格重写（`[S:open]`→`[S:partial][D:机] 建造完成·🟡 停等合入`，机制环境队列真身第 143 行，回读确认 4071 B ≤ `ROW_LENGTH_CAP_BYTES` 4096 B）；同批做 K2 行长外置——原 5596 B 的中间两段（2026-09-08 起草与 design 放行段、09-09 答 `2a` 归属段）原文原样迁 `1-转型规划/0-全景路线图/队列行日志/#504.md`（md5:`843d015a47f63542e6aae8c691990884`，回读比对一致），行内留首段＋指针＋新末段。§二 批次 `B-0909_504心跳apply回写` 第 319 行、态「待 sweep 取活」（🔴 由 `ZhuopinCommitSweep` 取活销行，不手动 commit 主仓）。全程走编辑锁 `acquire`／`edit-row`／`append-row`／`release`（`--who CC-OP-0909-M2`，退出码逐步查验，release `--waiver` 点名放行他线脏文件）。
