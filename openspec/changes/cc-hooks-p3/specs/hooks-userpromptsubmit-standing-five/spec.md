...
🔴 **连续性检查 MUST 有编号合理上限护栏**（防御性，非业务判据）：`\d+` 无上界，一个笔误 `UPS5:999999999` 会让缺号检查去枚举 1..十亿 ⇒ 钩子挂死，而 **UserPromptSubmit 挂死＝每一轮对话都卡住**，比它要防的告警噪声严重得多。超过上限的编号 MUST 报为"编号异常偏大"并 MUST NOT 参与枚举。编号解析 MUST 用 `TryParse` 而非直转 `[int]`——超出 32 位会抛异常、把整枚钩子打进 catch 分支，常驻纪律一条都注入不了。

#### Scenario: 编号异常偏大
- **WHEN** 出现 `UPS5:999999999`
- **THEN** 钩子在有限时间内 `exit 0`，报"编号异常偏大：999999999"，其余条目照常注入，审计记录 `verdict=undetermined`

#### Scenario: 编号溢出 32 位整数
- **WHEN** 出现 `UPS5:99999999999999999999`
- **THEN** 报"编号无法解析：…"，钩子 MUST NOT 走进自身报错分支，其余条目照常注入
