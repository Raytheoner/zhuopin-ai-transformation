...
> 执行泳道：`OP-0908-E`／批 `B-0908_B`／队列 §一 `#491`；分支 `claude/op0908e-general-readonly`。　**续段 `OP-0908-J`（批 `B-0908_B` 波 3）承接 §5 落字**，分支 `op0908j-work`（自 `origin/claude/op0908e-general-readonly` @ `e8023a7` 起）。
...
## 5. 隔离规范 §5 映射表 §2.3 行改判——**已落字**（Shao Peishen 2026-09-08 答 2a）
...
- [x] 5.2 ✅ **已落字**。**依据＝Shao Peishen 2026-09-08 答 2a**（原话：批准隔离规范 §5 映射表 §2.3 行改判（待设计→部分实现）并在 §6 追 V1.3 修订记录，与同表 §3.2 行 V1.2 同口径；批准人＝Shao Peishen 本人）。落字三处，全部在 `3-治理与合规/OEM数据隔离规范.md`：⑴ §5 映射表 **§2.3 行** 由 `❌ 待设计` 改判为 `⚠️ 部分实现`（逐字采用下方「拟改为」文本，零改动）；⑵ §6 修订记录新增 **V1.3** 行，**批准栏＝`Shao Peishen`**（照 §3.2 行 V1.2 同口径写法）；⑶ 按 V1.2 先例同批更新 §5 表头现状日期（`…§3.2 行 2026-09-07 更新，§2.3 行 2026-09-08 更新`）。落字前已比对：现文与下方「现文」块 **sha256 逐字相同**（`f6147fe006d846da…`），无他人改动漂移；`guard()` `router.py` L142-144 行号现取复核为通用库无条件放行块，属实。执行泳道 `OP-0908-J`【CC】
...
**§6 修订记录拟新增行**（草稿留空批准栏；🔴 **落字时按他 2026-09-08 答 2a 填入批准人 `Shao Peishen`**，并照 V1.2 同口径补「同批更新 §5 表头现状日期。」一句——落字实况以规范正文 §6 V1.3 行为准）：
