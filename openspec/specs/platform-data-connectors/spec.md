...
#### Scenario: 替代料用量恒继承主件行，不采信自身占位值（2026-07-15 真实数据验证）
- **WHEN** 提取某条替代料的 `BomRow`
- **THEN** `qty_per_unit`/`loss_rate` 恒取自其所属主件行，MUST NOT 读取/采信替代料 DTO 自身的 `m_usageQty`/`m_scrap`（生产只读实测确认该字段在替代料 DTO 上恒为占位值 `1.0`/`0.0`，与主件行真实用量无关，采信会导致该料位齐套判定严重低估真实需求）
