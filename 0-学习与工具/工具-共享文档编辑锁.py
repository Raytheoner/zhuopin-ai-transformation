...
# ── ⑷ 人的属性（性别代词）lint ───────────────────────────────────────
# 🔴 **本常量的唯一权威来源是 `6-人才与组织/人员名录-称谓与性别-正本.md`
# §一「名录」**（人员名录与性别；2026-08-28 由根 `CLAUDE.md` §1 迁出，
# OP-0828-Q，队列 #433 A2，CHANGELOG 附录 G-5，根文件此后只留指针）。
# 名录变动一律先改正本文件，再改这里；`test_工具-共享文档编辑锁.py` 里
# 有一条同步用例会从正本 §一 抽取全部带性别标注的姓名与本常量对表，
# **名录扩了而这里没跟，回归当场变红**。
#
# **刻意不在运行时解析正本文件**：§一 是每周都在变的散文，措辞一变解析
...
PERSON_GENDER_ROSTER: dict[str, str] = {
    ...
    "叶燕": "女", "齐奇": "女", "汤丽萍": "女",
    "袁洋": "男", "刘伟": "男", "聂鑫": "男",
    # 决策人本人（正本 §一 中以「`邵培申` ＝ Shao Peishen 本人」形式记载，
    # 无「（男）」标注，故同步用例走「正本 §一 ⊆ 常量」方向，不要求相等）
    "邵培申": "男",
}
