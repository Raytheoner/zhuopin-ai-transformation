...
def _detect_shadow_copy(target: str) -> str | None:
    ...
    return (
        ...
        "内容不一致。\n"
        "   本工具（含 release 的行身份守卫）看的**恒定是权威文件那一份**；\n"
        "   你在本 worktree 内的编辑，本工具此刻看不到。\n"
        "   ⇒ 若你正在 worktree 里改队列，这是**正常且唯一走得通**的路（协议〇.7）：\n"
        "     ① 就在本 worktree 内改自己那份 → ② 分支 ff 进 master → ③ 由落库 sweep\n"
        "        同步回主工作区。锁照占照放，它的协作信号（谁在改、改了什么）仍然有效。\n"
        "   🔴 但要知道代价：release 的行身份守卫此刻校验的是一份你没动过的文件，\n"
        "      **对你恒真通过、零信息量**——它没拦住你不等于你写对了，请自行核对\n"
        "      新增行的编号归属与状态列（队列 §一 #426）。\n"
        "   ⚠️ 若你并非 worktree 隔离、而是编辑器指错了文件（2026-08-10 #321 事故\n"
        "      形态），那才是真幽灵副本：两份都在改、后写的静默覆盖先写的。\n"
        "      区分方法只有一条——问自己「我打算让哪一份进 master」。"
    )
