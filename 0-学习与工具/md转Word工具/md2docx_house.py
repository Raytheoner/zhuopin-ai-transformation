...
class Conv:
    def __init__(self, doc, diagram_map, imgdir):
        self.doc=doc; self.dmap=diagram_map; self.imgdir=imgdir
    def add_image(self, key):
        ...
        run=p.add_run()
        try:
            run.add_picture(svg if os.path.exists(svg) else png, width=Inches(wi))
        except Exception:
            run.add_picture(png, width=Inches(wi))
    def heading(self, level, text):
        stylename='Heading %d'%min(level,5)
        try:
            p=self.doc.add_paragraph(style=stylename)
        except KeyError:
            p=self.doc.add_paragraph()
        p.paragraph_format.space_before=Pt(13 if level<=2 else 9)
        p.paragraph_format.space_after=Pt(6)
        size={1:16,2:14,3:12.5,4:11.5}.get(level,11)
        col=NAVY if level<=2 else NAVY2
    ...
    def para(self, text):
        ...
        add_runs(p, text, size=10.5); return p
    ...
    def numbered(self, text):
        p=self.doc.add_paragraph(style='List Number'); p.paragraph_format.space_after=Pt(3)
        add_runs(p, text, size=10.5)
    def callout(self, text):
        p=self.doc.add_paragraph(); p.paragraph_format.space_before=Pt(4); p.paragraph_format.space_after=Pt(8)
        shade(p._p, CALLBG); para_left_bar(p, NAVY)
    ...
    def table(self, rows):
        ...
        t=self.doc.add_table(rows=0, cols=len(header)); t.alignment=WD_TABLE_ALIGNMENT.CENTER
        t.autofit=True
        hr=t.add_row().cells
...
def parse(md_path, doc, diagram_map, imgdir):
    ...
    raw=re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F]','',raw)  # 去除控制字符，避免 XML 报错
    lines=raw.split('\n')
    i=0; n=len(lines)
    ...
    while i<n:
        ...
        if s.startswith('```'):
            ...
            content='\n'.join(buf); key=None
            for sig,k in diagram_map.items():
        ...
        if mb:
            ...
            conv.bullet(mb.group(2), lvl); i+=1; continue
...
def add_footer(sec):
    ...
    r=fp.add_run("第 "); set_font(r,CJK)
    fld(fp,"PAGE")
    r=fp.add_run(" 页 / 共 "); set_font(r,CJK)
    fld(fp,"NUMPAGES")
    r=fp.add_run(" 页"); set_font(r,CJK)
    for rr in fp.runs:
        rr.font.size=Pt(8); rr.font.color.rgb=RGBColor.from_string("999999")
...
def build(md_path, out_path, title, subtitle, diagram_map, imgdir):
    doc=Document()
    ...
    r=tp.add_run("卓品智能科技股份有限公司"); r.bold=True; r.font.size=Pt(12); set_font(r,CJK)
