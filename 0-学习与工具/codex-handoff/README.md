> 2026-09-25现时状态：主仓普通身份sandbox、正常信任hooks、新工作树继承及三消费者规定范围原生E2E均已通过；轮询任务按用户批准采用InteractiveToken（仅账户已登录时运行）。末轮注册器修复待自动落库，#648及业务开工闸暂不关闭；真实业务自动消费和五项Codex自动化保持暂停。证据见OpenSpec acceptance.md末节。以下历史记录不代替现时状态。
