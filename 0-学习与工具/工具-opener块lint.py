...
| 形态⑩ | **子任务泳道 opener 块**缺**心跳约定行**（须含 `heartbeat` 与 `--lane`） | 2026-09-12（队列 §一 `#565`／`OP-0912-B`） | 看护批 `B-0911_机制收口` 5 条泳道全做完全 ff 进 master，`summary --batch B-0911_机制收口` 却报「本批终态泳道 0 条」、`reports/lane-heartbeat/` 两小时零新文件——心跳约定此前只写在看护件「硬边界继承」段（看护者自己读）与 SKILL.md 步骤 4，两处都不是子任务会读到的 opener 正文。同批复核还发现 `heartbeat --done` 若不带 `--batch` 就不计入任何批次 `summary`（已实测撞过 8 条历史泳道），修法主体＝生成器强制注入（`SUBTASK_HEARTBEAT_NOTE`，含 `--batch`），本形态是它的机器守 |
...
#: 形态⑩ ＝ 队列 §一 `#565`（`OP-0912-B`，2026-09-12：子任务泳道 opener 缺心跳约定行）。
...
#: 形态⑩：心跳约定行——**同一行**须同现 `heartbeat` 与 `--lane`（队列 §一 `#565`）。
#: 🔴 只判「有没有」、判不了「措辞对不对」（同 F9 的弱校验哲学）：`工具-泳道看护状态机.py
#: summary`／`check-heartbeat` 看门狗全靠心跳文件判泳道死活，opener 正文没把这条约定告诉
#: 子任务 ⇒ 子任务无从知道要跑这条命令——2026-09-12 看护批 `B-0911_机制收口` 5 条泳道
#: 全做完全 ff、心跳两小时零新增即此坑的实撞。
#: 🔴 只判子任务泳道块（同 F6／F8／F9 的收窄）：标准【CC】／guardian／【Cowork】块不经
#: 批处理器的心跳看门狗判据，加了反而是噪音。
HEARTBEAT_LINE_RE = re.compile(r"heartbeat.*--lane|--lane.*heartbeat")
...
def check_block(block: Block, *, is_subtask_lane: bool = False,
    ...
    # 形态⑩ —— 子任务泳道 opener 块缺心跳约定行（队列 §一 `#565`，2026-09-12）：
    # `工具-泳道看护状态机.py summary`／`check-heartbeat` 看门狗全靠心跳文件判泳道死活；
    # 正文不告诉子任务这条约定，活做完了看护者也一无所知（2026-09-12 看护批
    # `B-0911_机制收口` 5 条泳道全中：全做完全 ff、心跳两小时零新增）。生成器已强制注入
    # （`SUBTASK_HEARTBEAT_NOTE`），本形态守的是「正本改了而生成器没跟」与
    # 「起草人手抄漏了」两种漂移。
    if is_subtask_lane and not any(HEARTBEAT_LINE_RE.search(ln) for ln in block.lines):
        problems.append((
            "F10",
            "子任务泳道 opener 块缺**心跳约定行**（须同现 `heartbeat` 与 `--lane`）"
            "⇒ 泳道做完活，看护者的 `summary`／看门狗一无所知（2026-09-12 看护批 "
            "`B-0911_机制收口` 5 条泳道全中）。用 `工具-opener生成.py --variant subtask_lane` "
            "重出，生成器会自动带上该行；手抄请照骨架【CC · 子任务泳道】块对应行（队列 §一 `#565`）",
        ))
...
FORM_TITLE = {
    ...
    "F10": "形态⑩ · 子任务泳道块缺心跳约定行 heartbeat／--lane（规则生效日 2026-09-12）",
