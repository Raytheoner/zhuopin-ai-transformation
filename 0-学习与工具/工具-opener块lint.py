"""opener 代码块 lint —— 一次收十六个失效形态（队列 §一 `#284`／`#381`⑸ⓖ／`#487`／`#550`／`#565`／`#620`／`#616`／`#627`，OP-0828-Y／OP-0904-A／OP-0905-C／OP-0906-I／OP-0909-X／OP-0910-R／OP-0912-B／OP-0918-C／OP-0919-G／OP-0920-N）。
...
## 十六个形态（判据正本＝`1-转型规划/0-全景路线图/opener骨架.md`；判据说明＝`专线opener模板库.md` §〇.0／§〇.00）
...
| 形态⑭ | **子任务泳道 opener 块**缺**回归 narrow 尾句行**（同一行须同现「受影响测试类」、禁 `-k`、「不起后台任务轮询等待」） | 2026-09-20（队列 §一 `#627`／`OP-0920-N`） | 2026-09-20 当日七条无头泳道全部越过 150k 转场线、零条执行 `OPENER_PARTIAL`——三条 PARTIAL 的理由全写成「回归没跑完」，根因＝泳道自己重跑整份「与纯 master 比失败集合」（那是 ff 六关④的活，泳道再跑一遍＝同一件事做三遍）外加起后台任务轮询等待，两者合力把会话耗到顶（`OP-0920-K` 第一段被 25 分钟耗死）。修法主体＝生成器强制注入（`SUBTASK_NARROW_TEST_NOTE`），本形态是它的机器守 |
| 形态⑮ | **子任务泳道 opener 块**缺**产出实证尾句行**（同一行须同现 `git diff --name-only master...HEAD` 与 `OPENER_PARTIAL`） | 2026-09-20（队列 §一 `#627`／`OP-0920-N`） | 同日 `OP-0920-K` 报 `OPENER_DONE` 而本体一个字没改（commit message 写成改了别的）——哨兵自陈不可信，须有机器可核的证据（`git diff --name-only` 与期望产出清单比对）才许写 `OPENER_DONE`。修法主体＝生成器强制注入（`SUBTASK_EVIDENCE_NOTE`），本形态是它的机器守 |
| 形态⑯ | **子任务泳道 opener 块**缺**新增告警类沙箱夹具尾句行**（同一行须同现「沙箱夹具要求」「是否影响其它沙箱测试」「用什么核的」） | 2026-09-20（队列 §一 `#627` B 面／`OP-0920-N`） | 第 19 类常驻状态告警上线后 `ScheduledTaskMirrorSyncTests` 三条断言全红，查明是夹具缺桩非门控逻辑错——`SUBTASK_EVIDENCE_NOTE` 此前只有一句散文提醒、没有自己的机器守，同一批下一个新增告警类照样能漏。修法主体＝生成器强制注入（`SUBTASK_ALERT_FIXTURE_NOTE`），本形态是它的机器守 |
...
#: 形态⑧ ＝ 队列 §一 `#487` 追记⑴（`OP-0909-X`，2026-09-09 apply：子任务泳道占位符未替换）。
RULE_EFFECTIVE_FORM8 = date(2026, 9, 9)
#: 形态⑨ ＝ 队列 §一 `#550`（`OP-0910-R`，2026-09-10：子任务泳道 opener 缺收工哨兵行）。
...
#: 形态⑭ ＝ 队列 §一 `#627`（`OP-0920-N`，2026-09-20：子任务泳道 opener 缺回归 narrow 尾句）。
RULE_EFFECTIVE_FORM14 = date(2026, 9, 20)
#: 形态⑮ ＝ 队列 §一 `#627`（`OP-0920-N`，2026-09-20：子任务泳道 opener 缺产出实证尾句）。
RULE_EFFECTIVE_FORM15 = date(2026, 9, 20)
#: 形态⑯ ＝ 队列 §一 `#627` B 面（`OP-0920-N`，2026-09-20：子任务泳道 opener 缺新增告警类沙箱夹具尾句）。
RULE_EFFECTIVE_FORM16 = date(2026, 9, 20)
...
RULE_EFFECTIVE_BY_FORM = {
    ...
    "F14": RULE_EFFECTIVE_FORM14,
    "F15": RULE_EFFECTIVE_FORM15,
    "F16": RULE_EFFECTIVE_FORM16,
...
#: 形态⑧：**未替换的正文占位条目**——整行只有一个列表记号 ＋ 省略号，别无内容
#: （队列 §一 `#487` 追记⑴，2026-09-09）。`1. …`／`- …`／`* …` 三种写法皆认，
#: 半角三点 `...` 与全角 `…` 皆认（骨架写全角，人手抄常写半角）。
#: 🔴 **必须整行锚定**：`1. …（细节见队列行）` 这类「占位符后面真写了东西」的行
#: 不该命中——它已经是填过的内容，只是作者用省略号起头。
PLACEHOLDER_ITEM_RE = re.compile(r"^\s*(?:\d+[.、)]|[-*+])\s*(?:…+|\.{3,})\s*$")

#: 🔴 **形态⑧只判子任务泳道块，这是刻意收窄、不是漏判**（同形态①「只对 CC 侧成立」
#: 那条收窄的性质）：
#:   - 骨架【CC · 子任务泳道】节**明写**「本变体恒为三行，不多写一行」——该变体的
#:     成品里根本不该出现正文段，出现了占位条目 ⇒ 一定是没填的模板被当成品发出去；
#:   - 而【Cowork】／【CC】标准变体的骨架**本来就有**这两段，未传 `--dont` 时留一个
#:     占位段是 2026-09-06 `OP-0906-M` 的**明示设计**（单测
#:     `test_cowork_未传dont时仍出段占位_不违反形态七` 把它钉死）——拿形态⑧去判它们
#:     等于推翻一条已判过的决定，须另走一次判据确认，本形态不擅自扩面。
#: ⚠️ **留下的射程缺口如实记在这里**：标准变体里 `1. …` 仍会静默出件，本形态**判不到**。
FORM8_SCOPE_NOTE = "形态⑧只判子任务泳道块（骨架该节明写「恒为三行」）；标准变体的占位段是明示设计，不判。"

#: 形态⑨：收工哨兵行——**同一行**须同现 `OPENER_DONE` 与 `OPENER_PARTIAL`（队列 §一 `#550`）。
#: 🔴 只判「有没有」、判不了「措辞对不对」（同 F2 的弱校验哲学）：`工具-opener批处理执行v2.ps1`
#: 认的是子任务**输出**的顶格 `OPENER_DONE`／`OPENER_PARTIAL`，而本形态判的是 opener **正文**
#: 有没有把这条收工协议告诉子任务——两个词缺一个，子任务要么不知道做完该说什么、要么不知道
#: 停在决策点该说什么，批处理器照样判 `NO-SENTINEL`。
#: 🔴 只判子任务泳道块（同 F6／F8 的收窄）：标准【CC】／guardian／【Cowork】块是人粘贴进交互
#: 会话的，不经批处理器的哨兵判据，加了反而是噪音。
SENTINEL_LINE_RE = re.compile(r"OPENER_DONE.*OPENER_PARTIAL|OPENER_PARTIAL.*OPENER_DONE")

#: 形态⑩：心跳行——**同一行**须同现心跳工具子命令 `工具-泳道看护状态机.py heartbeat`、`--done`
#: 与 `--batch`（队列 §一 `#565`）。三者缺一：缺子命令＝子任务不知道该跑什么（会回到「自己拼
#: 相对路径写进各自 worktree」的 `#504` 旧坑）；缺 `--done`＝泳道永不置终态，看门狗把收工后的
#: 静默判成失联；缺 `--batch`＝终态泳道归属未知，`summary --batch` 照报 0（`#536`）——正是
#: `B-0911_机制收口` 的现象本身。与 F9 同为弱校验：只判「有没有」，不判措辞。
#: 🔴 判据沿革：对照棒（`6733cb4`）为「同一行同现 `heartbeat` 与 `--lane`」；`OP-0912-F`
#: （2026-09-12，Shao Peishen 回 `2a` 全量移植第二棒）收紧为现形，规则生效日不变。
#: 🔴 只判子任务泳道块（同 F6／F8／F9 的收窄）：标准【CC】／guardian／【Cowork】块不经看门狗。
HEARTBEAT_LINE_RE = re.compile(
    r"(?=.*工具-泳道看护状态机\.py heartbeat)(?=.*--done)(?=.*--batch)")

#: 形态⑭：回归 narrow 尾句——**同一行**须同现「受影响测试类」、禁 `-k` 的提示与「不起
#: 后台任务轮询等待」（队列 §一 `#627`）。三者缺一：缺「受影响测试类」＝泳道不知道该
#: narrow 到哪；缺「禁 `-k`」＝仍可能整份重跑（`-k` 会 `exit=9` 中断，同 `.claude/rules/
#: 环境保障线.md`「回归闸 narrow」判据）；缺「不起后台任务轮询等待」＝会话仍可能被自建
#: Monitor／轮询耗到顶（2026-09-20 七条无头泳道全撞）。与 F9/F10 同为弱校验：只判
#: 「有没有」，不判措辞。只判子任务泳道块（同 F6／F8／F9／F10 的收窄）。
NARROW_TEST_LINE_RE = re.compile(
    r"(?=.*受影响测试类)(?=.*禁.*-k)(?=.*不起后台任务轮询等待)")

#: 形态⑮：产出实证尾句——**同一行**须同现 `git diff --name-only master...HEAD` 与
#: `OPENER_PARTIAL`（队列 §一 `#627`）。缺此行 ⇒ 收工是否写 `OPENER_DONE` 只能靠哨兵
#: 自陈，2026-09-20 `OP-0920-K` 即实撞「报 `OPENER_DONE` 而本体一个字没改」。与 F9/F10
#: 同为弱校验：只判「有没有」，不判措辞。只判子任务泳道块（同 F6／F8／F9／F10 的收窄）。
EVIDENCE_LINE_RE = re.compile(
    r"(?=.*git diff --name-only master\.\.\.HEAD)(?=.*OPENER_PARTIAL)")

#: 形态⑯：新增告警类沙箱夹具尾句——**同一行**须同现「沙箱夹具要求」「是否影响其它沙箱
#: 测试」「用什么核的」（队列 §一 `#627` B 面）。缺此行 ⇒ `SUBTASK_EVIDENCE_NOTE` 里那句
#: 散文提醒没有机器守，第 19 类 `ScheduledTaskMirrorSyncTests` 三条断言全红那类夹具缺桩
#: 会在下一个新增告警类身上重演。与 F9/F10/F14/F15 同为弱校验：只判「有没有」，不判措辞。
#: 只判子任务泳道块（同 F6／F8／F9／F10／F14／F15 的收窄）。
ALERT_FIXTURE_LINE_RE = re.compile(
    r"(?=.*沙箱夹具要求)(?=.*是否影响其它沙箱测试)(?=.*用什么核的)")
...
def check_block(block: Block, *, is_subtask_lane: bool = False,
    ...
    """返回该块命中的 `(形态代码, 说明)` 列表。形态代码 ∈ {"F1".."F16", "C3", "C5"}。
    ...
    # 形态⑧ —— 子任务泳道 opener 块里仍有**未替换的正文占位条目**（队列 §一 `#487`
    # 追记⑴，2026-09-09）：骨架【CC · 子任务泳道】明写「恒为三行，不多写一行」，
    # 做什么／不做什么一律写进队列行；出现 `1. …／- …` ⇒ 没填的模板被当成品发出去。
    # 🔴 形态⑦的**镜像**：⑦ 是「参数被接受却不生效」，⑧ 是「参数没传却仍产出内容」，
    # 根因同为格式正本与生成器之间没有机器守（本形态即那道守，见 `FORM8_SCOPE_NOTE`）。
    if is_subtask_lane:
        stale = [ln.strip() for ln in block.lines if PLACEHOLDER_ITEM_RE.match(ln)]
        if stale:
            problems.append((
                "F8",
                f"子任务泳道 opener 块里有 {len(stale)} 条**未替换的正文占位条目**"
                f"（如 `{stale[0]}`）⇒ 没填的模板被当成品发出去。骨架【CC · 子任务泳道】"
                "明写「本变体恒为三行，不多写一行」——做什么／不做什么一律写进**队列行**；"
                "要么删掉这两段、要么把真内容填进去（队列 §一 `#487` 追记⑴，2026-09-09）",
            ))

    # 形态⑨ —— 子任务泳道 opener 块缺收工哨兵行（队列 §一 `#550`，2026-09-10）：
    # `工具-opener批处理执行v2.ps1` 判成败靠 `claude` 退出码 ＋ 顶格哨兵两个指标；正文不告诉
    # 子任务这条收工协议，活做完了也会被判 NO-SENTINEL。生成器已强制注入（`SUBTASK_SENTINEL_NOTE`），
    # 本形态守的是「正本改了而生成器没跟」与「起草人手抄漏了」两种漂移。
    if is_subtask_lane and not any(SENTINEL_LINE_RE.search(ln) for ln in block.lines):
        problems.append((
            "F9",
            "子任务泳道 opener 块缺**收工哨兵行**（同一行须同现 `OPENER_DONE` 与 `OPENER_PARTIAL`）"
            "⇒ `工具-opener批处理执行v2.ps1` 会把做完的活判成 NO-SENTINEL 并中断本泳道"
            "（2026-09-10 四条泳道全中）。用 `工具-opener生成.py --variant subtask_lane` 重出，"
            "生成器会自动带上该行；手抄请照骨架【CC · 子任务泳道】块末行（队列 §一 `#550`）",
        ))

    # 形态⑩ —— 子任务泳道 opener 块缺心跳行（队列 §一 `#565`，2026-09-12）：看护批
    # `B-0911_机制收口` 五条泳道零心跳——不是不遵守，是心跳命令只写在看护者读的那两处
    # （SKILL.md 步骤 4／看护件 §一），子任务拿到的 prompt 里一个字没有。生成器已强制注入
    # （`subtask_heartbeat_note`），本形态守「正本改了而生成器没跟」与「起草人手抄漏了」。
    if is_subtask_lane and not any(HEARTBEAT_LINE_RE.search(ln) for ln in block.lines):
        problems.append((
            "F10",
            "子任务泳道 opener 块缺**心跳行**（同一行须同现 `工具-泳道看护状态机.py heartbeat`、"
            "`--done` 与 `--batch`）⇒ 该泳道全程零心跳：看门狗 `check-heartbeat` 无信号可判、"
            "`summary --batch` 报「终态泳道 0 条」（2026-09-12 凌晨 `B-0911_机制收口` 五条全中）。"
            "用 `工具-opener生成.py --variant subtask_lane` 重出，生成器会自动带上该行"
            "（泳道标识＝worktree 名，批次从 `--line` 现取或 `--batch` 显式给）；"
            "手抄请照骨架【CC · 子任务泳道】块对应行（队列 §一 `#565`）",
        ))

    # 形态⑭ —— 子任务泳道 opener 块缺回归 narrow 尾句行（队列 §一 `#627`，2026-09-20）：
    # 2026-09-20 七条无头泳道全部越过 150k 转场线、零条执行 `OPENER_PARTIAL`——三条 PARTIAL
    # 的理由全写成「回归没跑完」，根因＝泳道自己重跑整份回归（那是 ff 六关④的活）外加起
    # 后台任务轮询等待。生成器已强制注入（`SUBTASK_NARROW_TEST_NOTE`），本形态守「正本改了
    # 而生成器没跟」与「起草人手抄漏了」两种漂移（同 F9/F10）。
    if is_subtask_lane and not any(NARROW_TEST_LINE_RE.search(ln) for ln in block.lines):
        problems.append((
            "F14",
            "子任务泳道 opener 块缺**回归 narrow 尾句行**（同一行须同现「受影响测试类」、"
            "禁 `-k` 与「不起后台任务轮询等待」）⇒ 泳道可能重跑整份回归或起后台轮询耗死会话"
            "（2026-09-20 七条无头泳道全撞，零条执行 `OPENER_PARTIAL`）。用 "
            "`工具-opener生成.py --variant subtask_lane` 重出，生成器会自动带上该行；"
            "手抄请照骨架【CC · 子任务泳道】块对应行（队列 §一 `#627`）",
        ))

    # 形态⑮ —— 子任务泳道 opener 块缺产出实证尾句行（队列 §一 `#627`，2026-09-20）：
    # 同日 `OP-0920-K` 报 `OPENER_DONE` 而本体一个字没改——哨兵自陈不可信，须有机器可核的
    # 证据。生成器已强制注入（`SUBTASK_EVIDENCE_NOTE`），本形态守「正本改了而生成器没跟」
    # 与「起草人手抄漏了」两种漂移（同 F9/F10）。
    if is_subtask_lane and not any(EVIDENCE_LINE_RE.search(ln) for ln in block.lines):
        problems.append((
            "F15",
            "子任务泳道 opener 块缺**产出实证尾句行**（同一行须同现 "
            "`git diff --name-only master...HEAD` 与 `OPENER_PARTIAL`）⇒ 收工写不写 "
            "`OPENER_DONE` 只能靠哨兵自陈，2026-09-20 `OP-0920-K` 即实撞「报 `OPENER_DONE` "
            "而本体一个字没改」。用 `工具-opener生成.py --variant subtask_lane` 重出，"
            "生成器会自动带上该行；手抄请照骨架【CC · 子任务泳道】块对应行（队列 §一 `#627`）",
        ))

    # 形态⑯ —— 子任务泳道 opener 块缺新增告警类沙箱夹具尾句行（队列 §一 `#627` B 面，
    # 2026-09-20）：第 19 类常驻状态告警上线后 ScheduledTaskMirrorSyncTests 三条断言全红，
    # 查明是夹具缺桩非门控逻辑错——`SUBTASK_EVIDENCE_NOTE` 此前只有一句散文提醒、没有
    # 自己的机器守，下一个新增告警类照样能漏。生成器已强制注入
    # （`SUBTASK_ALERT_FIXTURE_NOTE`），本形态是它的机器守（同 F9/F10/F14/F15）。
    if is_subtask_lane and not any(ALERT_FIXTURE_LINE_RE.search(ln) for ln in block.lines):
        problems.append((
            "F16",
            "子任务泳道 opener 块缺**新增告警类沙箱夹具尾句行**（同一行须同现「沙箱夹具"
            "要求」「是否影响其它沙箱测试」「用什么核的」）⇒ 新增常驻状态告警类时不知道要"
            "同批交代其它测试的沙箱夹具要求，可能重演第 19 类 `ScheduledTaskMirrorSyncTests` "
            "三条断言全红的夹具缺桩事故。用 `工具-opener生成.py --variant subtask_lane` "
            "重出，生成器会自动带上该行；手抄请照骨架【CC · 子任务泳道】块对应行"
            "（队列 §一 `#627` B 面）",
        ))
...
FORM_TITLE = {
    ...
    "F14": "形态⑭ · 子任务泳道块缺回归 narrow 尾句行（规则生效日 2026-09-20）",
    "F15": "形态⑮ · 子任务泳道块缺产出实证尾句行（规则生效日 2026-09-20）",
    "F16": "形态⑯ · 子任务泳道块缺新增告警类沙箱夹具尾句行（规则生效日 2026-09-20）",
...
def _run(args) -> int:
    ...
    if hist:
        print(f"\n── 历史件命中 {len(hist)} 处（F1 {sum(1 for f in hist if f.form == 'F1')} ／ "
              ...
              f"F7 {sum(1 for f in hist if f.form == 'F7')} ／ "
              f"F8 {sum(1 for f in hist if f.form == 'F8')}）：**按「历史记录不追改」不修、不阻断** ──")
