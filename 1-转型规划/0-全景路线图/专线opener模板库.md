...
2. **session 名**：🔴 **`[Win]MMDD-X<主题短名>`，不留空格**（Shao Peishen 2026-08-27 改定两次：先由 `[CC] OP-MMDD-X …` 改为 `[Win] MMDD-X …`，再去掉全部空格）。
   ⚠️ **唯一的边界**：**主题短名以 ASCII 字母/数字开头时，必须在编号后补一个 `-`** —— 否则 `0827-A` ＋ `master分叉` 会粘成 `0827-Amaster分叉`，编号尾字母与主题首字母无法分辨。**主题以汉字开头则直接接**（`0827-A波次收口` 一眼可辨）。
   🔴 **CC 与 Cowork 的设定手段不同，不要混用（2026-08-27 实测坐实）**：
   - **CC 侧**：代码块内调 `mcp__ccd_session_mgmt__set_session_title`（见补充三）。
   - **Cowork 侧**：**没有这个工具** —— 本方在 Cowork 会话内实测查找，返回的只有 `list_sessions`／`read_transcript`／`Task*`，**`mcp__ccd_session_mgmt__*` 一个都不存在**（Shao Peishen 提醒「`set_session_title` 好像对 Co 不适用，请小心」，实测属实）。**Cowork 的 session 名取自开场词首行** ⇒ **Cowork 开场词的第一行本身必须自带编号**（历史实证：`环境总线 OP0823B`／`OP0822F-A档下沉`／`【OP-0821-B】环境总线 memory治理0821` 全部带编号且都是这么来的）。
