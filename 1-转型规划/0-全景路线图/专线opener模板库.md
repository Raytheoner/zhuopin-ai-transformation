...
**事故**：`OP-0901-E` 在两次连续回复的「你要做的动作」小节里，把同一个 `[OP-0901-G]` 完整 opener 块贴了两遍——编号、`【设置】` 行、正文**逐字相同**。他当时已在跑第一份，看到第二份时问：「上面已经给过，我已在跑，你是否重复了？**会引导我执行两遍**」。**本次零损害**（他没开第二个，取证确认两条批次的 commit 已正常落地、工作区干净），但若照贴执行，就会出现**两个自称 `OP-0901-G` 的 session 抢同一批 §二 批次**。

🔑 **成因（要记的是这一层）**：写「你要做的动作」这一节时，做法是**从头重新罗列当前所有待办**，于是 opener 作为「待办的一部分」跟着被重列了一遍。**但 opener 不是状态、是动作**——状态可以反复陈述而无害，**动作重述一次就是重派一次**。
...
📌 **可机制化落点（达 3/3 阈值时用）**：给回复渲染层或看护件 lint 加一条——**同一会话内已出现过的 `[OP-MMDD-X]` 编号，再次出现在代码块里即告警**。判据是纯文本匹配，成本极低。
