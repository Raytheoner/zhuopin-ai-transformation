...
2. **代码块内 `set_session_title` 的标题**：🔴 **`[Win] MMDD-X <主题短名>`**（Shao Peishen 2026-08-27 改定，取代原 `[CC] OP-MMDD-X <主题短名>`）。
...
🔴 **补充三之二：标题有长度上限，超了先被吃掉的可能就是编号（Shao Peishen 2026-08-27 观察）**
他的判断：「**带编号的标题好像有总字数限定，多了自动吃掉编号，所以 opener 正文都要换行开始**」。
**⚠️ 如实标注证据强度**：本方**无法证明**这个机制（没有官方文档、也没做对照实验）。**但已有证据与它一致且无反例** —— 实测 24 条 session 名：带编号且总长 **≤ 约 20 字符**的 8 条（`【OP-0821-B】环境总线 memory治理0821`／`OP0822E-三份业务域接力件定长化`／`[OP-0821-M]质量域范围处置`／`OP0822F-A档下沉` 等）**编号全部保住**；而丢掉编号的那条（`0827-A` → 「波次收口 2026-08-26」）**其 opener 首行是全场最长的一行**。
**⇒ 三条照做，代价为零**：

1. **opener 首行只放标题，正文从下一行起** —— 不要把 `【设置】` 或任何正文挤在标题行后面。
2. **主题短名压到 ≤10 个汉字**，使 `[Win] MMDD-X <短名>` 总长控制在 **20 字符内**。
3. 🔑 **编号放最前面本身就是抗截断的**：万一真有上限，被吃掉的是**尾部主题**，不是**头部编号** —— 而主题丢了还能猜，编号丢了就没法对齐。**这是 `[Win] MMDD-X` 这个次序的第二个理由。**
