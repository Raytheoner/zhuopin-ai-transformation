...
created: 2026-09-08
status: 待粘贴（软序由会话开工第 1 步机判：op0908p 是否已为 master 祖先）
...
▶ 粘贴端：CC（新开）—— **随时可粘**：软序由会话自己机判（做什么 1），不靠人看 git log。机器判据＝`git merge-base --is-ancestor origin/claude/op0908p-opener-single origin/master` 退出码 0（泳道 `503-opener-single` 的分支已是 master 祖先＝已 ff）；≠0 ⇒ 会话 `pause` 报一句即停、零改动。
...
做什么：
1. 前置（机判，不问人）：`git fetch origin` 后跑 `git merge-base --is-ancestor origin/claude/op0908p-opener-single origin/master`；退出码 0 ⇒ 继续；≠0 ⇒ `python 0-学习与工具/工具-泳道看护状态机.py pause --no-notify` 报一句「软序未到：op0908p 未 ff」即停，不在旧基线上改、不轮询等待。
