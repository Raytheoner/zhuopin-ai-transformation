...
## 【附录 L · 规则退休声明 · 队列 §四 `#91`「等某某交付类未勾项须定期反查」—— 2026-09-12（`OP-0912-AA`，openspec 包 `tasks-zombie-item-detect` apply）】

> **退休对象**：队列 §四 `#91`（2026-08-21 立）那条人守规则——「凡『等某某交付』类未勾项，须定期反查它是否已被后续 design 超越」。100% 人守、无执行时点、无执行者；其前身已用 `fi2-recon-mvp` 一个包违反 7 次（形态 A/B/C 全是「没人回头反查」，证据在该包 `proposal.md` §Why）。
> **机制化承接**：`0-学习与工具/工具-僵尸未勾项lint.py` ＋ CI job `zombie-task-lint`（`.github/workflows/ci.yml`）。J-A 前置矛盾／J-C 前置登记项无下游引用＝强判据 `--enforce`；J-B 节内乱序只出「请复核」清单、永不进退出码；J-D 基线回显。design 决策点 4 由 Shao Peishen 2026-09-12 回 `P2：4a` 签认，其余四点按默认 (a)。
> **🔴 `#91` 原文不动**：该行已于 2026-09 清扫入 `跨桌任务队列-归档-202609.md`，按「历史记录不追改」不做 tasks 2.8 的「正文降级为一行指针」；退休声明只落本附录 ＋ 该包 `proposal.md`。「退休是否成立」（该包 Open Question 3）他尚未明答——本附录记的是**机制已上线、人守条目不再计入**这一事实，不是替他答。
> **不退休的一项**：「逐项人工核实」不退休——本工具对「节内最后一项 ＋ 无前置声明」这一形态够不着（`fi2` 的 `10.13`，召回 6/7），J-B 精度 4/5 且唯一误报是真未决项（`10.11b`）；工具是把该复核的项摆到眼前，不替人判。
> **apply 时实测（现取 79 个活跃包，非 design 表里的 14 个）**：前置声明 11 处／前置登记项 6 个；J-A 首跑 3（全部是 `status-triage-resident-round` §7 标题把「前置 6.3」声明得过宽——7.1–7.3 不依赖 6.3、已按实做完；已把该声明下移到 7.4，不改任何勾选）→ 0；J-C 0；J-B 47。


## 【附录 M · `.claude/rules/队列与落库.md` 瘦身前全文存档 —— 2026-09-13（`OP-0913-E`，Cowork 环境总线）】

> **成因**：落库 sweep「1 份必载 CLAUDE.md 超限」告警（2026-09-13 03:11 本地）——该文件 **9,889 B**、超阈值 8,192 B 共 **1,697 B**；同日 04:1x 本会话又为 `#521`／`#554`⑵／`#566` 三条 J1 承接载体各加一段成因，推到 **11,367 B**。该告警此前**在两份队列真身与归档件里查无任何承接行**（手段＝`工具-队列查询.py --digest --grep`，四个关键词命中 0），属「告警长期无人消费」一族。⇒ 本次不立新行（可动 WIP 已 24/22），由环境总线当场瘦身并把原文存进本附录。
>
> **瘦身口径**：该文件自身的规矩是「只写触发 → 动作 → 判据／指针；谁定、何时定、成因、实证一律在 CHANGELOG」，此前正文违反了这条。本次逐条只删**成因／实证／验活记录／谁定何时定**，**判据一条未删**——落地前用 78 条关键短语逐条 `in` 核对，缺失 0 条才允许写盘（核对表见本会话 commit 的脚本）。瘦身后 **8,150 B**（余量仅 42 B，下次再加句子前先量）。J1 承接载体＝本附录（具名文件＋章节）。
...
- **跨桌任务队列纪律**：`1-转型规划/0-全景路线图/跨桌任务队列-机制环境.md` ＋ `…-业务场景.md`（两份真身、共用一把锁；旧 `跨桌任务队列.md` 为纯指针）是两桌任务流转的**唯一载体**——**开工必读**（认领＋登记触碰区）、**收工必写**（状态＋产出路径＋下游任务当场追行）；触碰区与他人在办重叠不得抢领，报总线裁决。**commit 由 `ZhuopinCommitSweep` 自动从 §二「待 commit 批次」取活销行（约 27 分钟一轮）——登记完 §二即完事，不需要「一句话交 CC」**；CC 仅在 sweep 不可用时兜底。§三口径冻结标挂标即停该场景建造。🔑 **编辑锁／每周清扫／高水位线／并入审核／可动 WIP 等细则以队列顶部「协议〇」为正本。** 🔴 **可动 WIP 余量优先给需求侧（Shao Peishen 2026-09-06 定；上限 22 不动、协议〇.9 不改）**：值周巡检／总线去 🛑 的顺序＝先「需求侧」`[D:机]` 行（口径点台账 `#439`、跟进信元数据 `#447`、纪律 eval `#440`、需求侧机器守一族），后环境类行；判据一句＝该行建成后减少的是「以人的速度运行的环节」（问需求／等回件／确认使用）而非机器环节。 值周巡检每周一 10:00 出《本周计划》，开跑前先跑 skill `zhuopin-queue-audit`。**gap 收口**：设计↔执行 gap 须驱动到有主·有截止·已派单后计划才往下走。
...
- 🔴 **`登记豁免：` 止血口径（Shao Peishen 2026-09-07 令「及时止血、及时根治」；机器守＝`#416`⑶ 派单 `OP-0907-AM`，落地前人守）**：`release` 前工作区每个脏文件只有两条路——**登进 §二 批次**，或在 acquire note 里**逐个点名豁免**（`登记豁免：<仓库根相对路径>；<路径>…（作者 <OP 号>，到期 MM-DD）`）。🔴 **禁写「他线脏文件由其作者线自登」这类不点名的泛豁免**——09-07 16:21 实证：21 个孤儿来自 ≥5 个会话、每次 release 都这样放行、无一自登，sweep 只报不落库。看到 sweep 孤儿告警的会话，**先核 `git diff --numstat` 再代登**（只增日志与已核 diff 的留痕件可代登；载荷 `队列回写待补/*.json` 与在跑会话的件不代登）。
- 🔴 **定夺答复的落点：谁端出来的题，就在谁那儿答**（Shao Peishen 2026-09-08 答 2a 定，两桌全局）。Cowork 端的在 Cowork 答，CC 端的在 CC 答；**跨端转达一律标「已答，勿重做」并写明由谁执行**。🔴 **同一题不得两端各答一次**——两端的执行体互相看不见对方，会同时动手。**成因（实撞）**：2026-09-08 `B-0908_B` 第 13／14 项在 Cowork 与 CC 各答了一次 `14a`，两边同时准备 rebase＋ff 同四条分支，**差约 30 秒即撞车**（Cowork 侧因分支被既有 worktree 占用而未成，属侥幸不属机制）；同族先例＝本周计划 D-⑩「派出前 12 分钟已被另一线做完」。⚠️ **例外只有一个**：泳道看护批的 🟡 停点恒由 Cowork 端出并只认 Cowork 侧答复（lane-watch D5／tasks §8.1），该口径不因本条改变。
- 🔴 **批次命名：尾段必须是语义词、不得是单字母**（Shao Peishen 2026-09-08 定，两桌全局）——写 `B-0908_看护五泳道`，**不写 `B-0908_B`**。**成因**：09-08 当天 `OP-0908-B`（会话）与 `B-0908_B`（批次）并存、且看护者标题把两者拼在一起，他当场指出「一个 session 两个编号很容易搞混」。`工具-共享文档编辑锁.py` 的 `BATCH_NUMBER_PREFIX_RE = ^(B-\d{4}_[^_\s]+)` 只要求非下划线非空白 ⇒ **语义词照过，无需改工具**。🔴 **历史批次名不追改**。配套改动（opener 标题只放 OP 短号，不放批次号）与判据全文见 `1-转型规划/0-全景路线图/opener骨架.md`「编号脱形」节。
...
- 🔴 **规划倒逼（「规划里有、队列里没有」的那一棒，`#462`，2026-09-07 上线）**：判定正本＝`0-学习与工具/工具-规划倒逼扫描器.py`（三处查：队列 §一 任务列**标的段**／`4-数字员工/<部门>/` 一层目录／`openspec/changes` 含 archive）＋ 场景机读正本 `1-转型规划/0-全景路线图/场景registry-规划倒逼扫描器.jsonl`（改权威排期表必须同批改它，CI `--lint-registry` 硬失败）；常驻载体＝sweep **第 13 类**（每轮推，指纹不变即静默）。🔴 **它只出清单与队列行草案、一个字节都不写队列**（design D5＝(b)，Shao Peishen 2026-09-07 合审）——**落库前必须由人做一次协议〇.10 并入审核**；域级暂缓场景留清单、不推送、永不出草案。取证件 `取证件-2026-09-02-规划与执行覆盖差集.md` §三 那段脚本**已于同批退休**，勿再复制粘贴。
...
- **规则退休制**：纪律分「机制守」（有代码／门禁强制）与「人守」。**任一「人守」条目被违反 3 次即必须二选一**：① 机制化（挂到不可绕过的咽喉：队列写入＝编辑锁 `acquire/release`；提交＝sweep），正文降为一行指针；② 删除。违反计数落在队列对应行内。 🔴 **换机制前先证覆盖面是超集（`#554`② 正体；2026-09-13 由 Cowork 环境总线 `OP-0913-E` 从队列迁入，为该行关闭前的 J1 承接载体）**：**下线或替换任一机制前，必须先证明接替者的覆盖面是旧机制的超集**，证明写进下线那一笔的留痕；证不出即不下线。**成因（2026-09-10 实证）**：拆件巡逻下线后，事件驱动接替者只消费「在途信件收到回件」，旧巡逻消费的是「§一 全部待领归档行」⇒ **专员主动发言与对已闭环信件的补发从此无人消费**，五条行静默躺在 §一，靠 Shao Peishen 人眼看出才被发现、下线时无人察觉。判别一句：**「新机制跑通了」是抽样，「旧机制覆盖的都还在」要的是集合包含关系——前者永远证不出后者。** ⇒ 根问题与首例见 CHANGELOG 附录 E-1。
- 🔴 **sweep 第 16 类「未并入分支」B 类清单的消费口径（Shao Peishen 2026-09-11 答 `3a` 定；2026-09-13 由 `OP-0913-E` 从 `#521` 迁入——`#443` 每周清扫已 `done`，本条即其承接载体）**：第 16 类报出的 B 类「真未落地」分支**只出清单、不合不删**，交**每周清扫**按计数统一处理；分类判据正本＝`0-学习与工具/工具-未并入分支分类.py`（patch-id 只数 B 类，B ≥5 且最老 ≥7 天触发）。⚠️ B4 承接行 `#162`／`#315` 在两份队列真身与归档件均查无 ⇒ 清扫时按「无承接载体」单列，**不强行归因**。
- **接力卡 R5**：定长交接卡（8 KB、六块、零日期节、收工覆盖；进度→CHANGELOG、方法→取证知识库、未闭合→队列行，迁出守 J1；机器守＝J6）。

```

## 【附录 N · 根 CLAUDE.md §1／§3／§5 第六刀去 provenance 前全文存档 —— 2026-09-16（`OP-0916-G`，队列 `#583` Token 优化 Phase 5 第二段）】

> **成因**：`#583` 实测开场底噪（`#580` 177 条链首请求 P50 61.8k／P95 78.7k，×14,907 请求 ≈ 9.2 亿 ≈ cache_read 的 34%）；根 `CLAUDE.md`（18.5 KB）是唯一无条件随每轮请求进入上下文的项目级文件，其中 §5 常驻纪律五条各夹带完整「谁定／何时定／原话引用／分钟级事件时间线」的成因段，与文件头 "本文件只留常驻纪律＋路由表；成因一律在 CHANGELOG" 的自述口径长期脱节。本次瘦身**只删成因／实证／事件时间线，判据一条未删**——落地前逐条核对判据短语仍在新文本中才允许写盘。瘦身对象＝ §1 决策人称呼收窄段、§3 转场提醒条、§5「需你定夺」代码块成因、§5「代执行优先」成因与收窄成因、§5「会话不自醒」三段成因。原文如下（逐条为瘦身前根 `CLAUDE.md` 对应行原文，未改写）：

**§1 决策人（原 `CLAUDE.md` 行 17）**：
> - **决策人**：**Shao Peishen**（分管供应链与质量的 OPVP）。**称呼一律 `Shao Peishen`，不用 `Paul`**；🔴 **`Paul` 只在两处保留：① 真实 Windows 用户目录路径 `C:\Users\Paul Shao\…`（改了路径即失效）；② 计划任务运行身份字面量 `Paul Shao / S4U`**——**除此之外一律 `Shao Peishen`／`Peishen`，文件名亦然**（Shao Peishen 2026-09-08 收窄：仓库 2026-08-26 已迁出 `C:\Users\Paul Shao\OneDrive\…` 到 `C:\Dev\zhuopin-ai`，真路径只会越来越少，其余场合留 `Paul` 只会造成重名混淆）。🔴 **历史记录不追改**——判别：**已发生事实的原文**（commit message／已推送信件正文／带日期的历史快照件与归档队列行）**不改**；**当下生效的指针**（frontmatter `用途`／`配套`、活件里的路径与标题引用）**改**。已按此改名：`Paul标准工作周.md` → `Peishen标准工作周.md`（`OP-0908-B`，2026-09-08）。缺席时**孙涛**代理（范围见 rules/场景建造与合规）。
...
**§5「需你定夺」代码块成因（原行 56 尾段）**：
> **成因**：正文里的一行要靠手动划选，**既费一次操作，又可能选不全或多选**；代码块的复制按钮**取的是块内全文，不依赖选择**，所以最保险。

**§5「代执行优先」成因与收窄成因（原行 58）**：
> 🔴 **代执行优先（Shao Peishen 2026-09-09 定，两桌全局；排在决策路由之前，是它的上位过滤）**：…**成因（他原话「减少我人工动作，减少犯错机会」＋当日实证）**：`OP-0909-P` 派单件 16:0x 起草、17:2x 复核，**80 分钟内三项前置全部失效**（master 前进 21 笔／他棒已改好工作区／worktree 自行消失），且第 1 条「先 ff」在复核时已是**会反向删掉 737 行的危险动作** ⇒ **交给他的人工动作本身就是一个失效窗口，中间件越多、窗口越长、他按错的机会越多。**……🔴 **收窄一档（Shao Peishen 2026-09-11 定）**：……**成因**：2026-09-10 本方即凭印象把微盘取件写成「机器无法代」，未试过计算机使用通道。

**§5「会话不自醒」三段成因（原行 59）**：
> 🔴 **会话不自醒；说「自动」必须当场指名机器规则（Shao Peishen 2026-09-10 裁 (乙)，两桌全局；同日 17:1x 事实更正）**：……**成因（当日实证）**：本方 15:0x 承诺「盯着 `summary.txt`，一出来就自动起基建棒，不用你再喊」，两批 14:47 即收工，**直到 16:18 他主动问才被发现，空转 90 分钟**；同日另有一次「待锁空出后登记 §二」，忘了 2 小时、期间多次 acquire/release 都没想起，最后靠 release 的 ⑹ 登记完整性守卫逮住。🔴 **第三条成因（2026-09-10 17:1x 本方自查，也是本条被更正的由来）**：立本条时本方**凭印象**断言「没有定时器、没有回调」，**没查工具表就写进权威载体**——而 `mcp__scheduled-tasks__list_scheduled_tasks` 一跑即知 5 条在册。**用未核实的能力自陈去立法，比忘记更坏一档**：忘记只是漏一次，它把一个本可机器化的缺口**写死成「做不到」**，从此没人再去建那条机器规则。同开篇纪律「凡『已复核』类自陈必须写出用什么核的」——原成因段三句实证全是他那侧的现象，**关于本方能力的那一句一个手段都没写**。

> **瘦身结果**：根 `CLAUDE.md` 由 19,011 B → 目标 ≈10 KB（实得数字见本行落库时的 commit diffstat）；判据核对表＝上述五处判据短语（「谁去执行」「代价」「唯二可交给他的动作」「触发器只能是」「新机制跑通了是抽样」等）逐条 `in` 校验，全部命中方允许写盘。控制对照法重测（`claude -p "reply ok" --model haiku` 首请求 `cache_creation+cache_read+input`）：瘦身前一次探测 ≈42,269（详见 `reports/baseline-noise-breakdown-2026-09-16.md`）。

## 附录 P · 根 `CLAUDE.md`「当前进度」段（2026-09-20 迁入）

> 迁入成因：根 `CLAUDE.md` 2026-09-20 现取 13,148 B，超 `工具-落库sweep.py::CLAUDE_MD_ROOT_BYTE_CAP`（12,288 B）**860 B**；而该段是全文最大的单块（1,285 B），内容全是重组循环的九文档改动明细——正是本文件头规定「一律进 CHANGELOG」的那一类。迁出后根文件落到阈值内。**此后根文件该段只留指针，新批次直接写进本附录。**

### 2026-09-17 · 重组循环第 11 例（迁入时的最近一批，原文原样）

**当前进度**：本段只留最近一批。🔴 **2026-09-17 · 重组循环第 11 例（v10，同日晚于第 10 例）**：**八场景前拉 ＋ 四场景转 D 档**已落九文档——FI3 `2026-11→2026-09`｜O3 `2026-12→2026-10`｜FI5／FI6 `2027→2026-10`｜FI4／FI8／FI10 `2027→2026-11`｜FI7 `2027→2026-12`；SC4／SC10／SC11／FI9 转 **D 档（月份不构成承诺，实际启动＝前置到位＋8 天）**。**源＝Shao Peishen 2026-09-17 答 `1a 2a`**；依据件＝`1-转型规划/0-全景路线图/提案件-建设工期重估与排期前拉-2026-09-17.md`；承接载体＝队列 §一 `#602` ＋ §四 `#209`。🔑 **判据一句：建造不再是工期，口径收敛才是工期**（实测 骨架 ≈6 分钟/场景、propose→apply 中位 6.6 小时、泳道单条 26.5 分、SC2 端到端 6h59m 当天；专员回件 2.8 天/轮）。⚠️ **三处标待裁未代改**：S1 覆盖轨目标 9-11（前拉后实为 18）／前置总表 6 周·8 周纪律（与新模型同源不自洽）／七个前拉场景前置行缺失（卡 EE-5 待定项 3/4/6，不代填 Owner）。 ━━━ 〔前一批 · **2026-09-17 · 重组循环第 10 例（EE-6）**：`SC4` 启动月 2026-10 → 2026-11 已落九文档；承接载体＝队列 §一 `#467`〕

## 附录 Q · `.claude/rules/*.md` 六份瘦身前全文存档 —— 2026-09-21（队列 §一 `#629`「读文件瘦身」，第一棒五份＋续棒 `OP-0921-F` 补 Q-6）

> **成因**：`.claude/rules/*.md` 六份合计现取 **43,419 B**，超机制阈值 `工具-落库sweep.py::CLAUDE_MD_RULES_TOTAL_BYTE_CAP`（30,720 B）**约 12,700 B**；其中 `两桌同步与取证.md` 单份 **12,339 B**，超单份阈值 `CLAUDE_MD_RULES_BYTE_CAP`（8,192 B）**约 4,150 B**。瘦身口径同附录 M：六份文件头本已自述「只写触发→动作→判据／指针；成因／实证一律在 CHANGELOG」，本次是让正文真正遵守这条——逐条只删**成因／谁定何时定／原话引用／具体事故细节／实测数字叙事**，**判据／触发／动作一条不删**。落盘前用关键短语核对表逐条核对，缺一条不允许写盘。因总量缺口远大于单份超额，第一棒除主力目标 `两桌同步与取证.md` 外，`环境保障线.md`／`队列与落库.md`／`场景建造与合规.md`／`跟进信与专员.md` 四份也一并瘦身；`文档与全景治理.md`（4,243 B，本来就短）当时未动。**续棒 `OP-0921-F`** 因合计仍超阈值，把 `文档与全景治理.md` 也一并瘦身，其瘦身前原文补存 Q-6。六份瘦身前的**当前完整原文**逐字存档如下。

### Q-1 · `.claude/rules/两桌同步与取证.md`（瘦身前，12,339 B）
...
- 🔴 **口令「开启Opener OP-MMDD-X」（Shao Peishen 2026-09-10 定，答 1a／2a）**：他审过的 opener 由 Cowork 会话代起，他不再切 Code tab、不再粘贴。**【CC】opener ⇒ 无头**：Cowork 经 `0-学习与工具/工具-opener批处理执行v2.ps1` 起 `claude -p`（`-FullAuto`＝skip-permissions，**只在隔离 worktree 内**，护栏＝状态机四档＋hooks＋git 可回滚），起前必重跑前置实测（lint／触碰区／LAN／WIP／分支现状，不认起草时的表），先 `-DryRun -Yes` 核泳道数；单棒先包成单泳道 plan（A1 一节）；进展看 `reports/opener-batch/<时间戳>/*.log`＋心跳＋`summary`＋企微运维群，🟡 停点由 Cowork 以「需你定夺」端出、答后 `resume`；**盯细节／接管＝终端 `claude --resume <session-id>`**（无头起时带 `--session-id <uuid>` 与 `-n "[Win]MMDDX-<短名>"`，id 与名字写进 §二 批次与看护件，机器守落 `#522`）。**【Cowork】opener ⇒ 就地**：本会话读该件、按其【设置】与做什么／不做什么成为那条线；互动型（grill／值周答题／清扫逐行判）前台跑，非互动型派子代理、结果回本会话。**唯一仍需新开会话的＝转场 opener**（新上下文）。🔴 本口令不放松任何红线：🔴／⏭️／🟡 档处置一字不变。
- **执行环境判别**：只产改 `.md`、不写生产码、不连真实库 ＝ Cowork；写跑代码／连真实库与 `.51`／部署／一任务一 worktree ＝ CC（🔴 **2026-09-12 收窄：`commit` 不再是判别项**——Shao Peishen 当日答 `1a` 把 Cowork 侧改为**自行 commit+push**，与 CC 泳道拉平，两桌都提交，判别只看「改不改生产码／连不连真实库」。**成因**：此前 CC 泳道自己 commit+push、唯独 Cowork 要等 sweep，于是「派件必须先落库、泳道才看得见」这一步只能靠人**手动催一轮 sweep**——2026-09-12 一天手动跑了 4 次，`手动跑 sweep` 这个动作的唯一来源就是这条不对称。🔴 **守卫一条不放松**：Cowork 提交前仍须走 `工具-共享文档编辑锁.py` 的 `acquire/release`，登记完整性⑹、opener 守卫、称谓核对三道照跑；**§二 登记照登**，sweep 退为兜底核对而非唯一落库通道。）；只读取证属 Cowork，一旦改代码或触发服务动作即转 CC；**改本机工具链（npm/插件/CLI 升级、`openspec update` 类）＝ CC**，Cowork 破例须其明示授权＋固化升级前证据＋逐文件过目 diff＋事后登记。**「已获授权」≠「该照原方案执行」**——取证推翻方案即回报改判。
...
- 🔴 **`Claude outputs/` 不入库（Shao Peishen 2026-09-11 答 `a` 定，两桌全局）**：Cowork 每调一次 `SendUserFile` 交付文件，桌面端就在**连接文件夹根**（＝仓库根）自动落一份同名副本到 `Claude outputs/`。它是**交付通道的副产物、不是仓库资产**（权威件一律另在库内正式落点），已入 `.gitignore` ⇒ **此后不必再为它逐次 `release --waiver`**。🔑 **判别一句：凡由交付／预览通道在仓库根自动生成、且库内已有权威件的副本，一律入 `.gitignore`，不进 §二 批次。** **成因**＝`OP-0911-H` 2026-09-11 实撞：19:30 调 `SendUserFile` 交付 `财务部#18` 草稿，19:49 队列锁 `release` 即被 ⑹ 登记完整性守卫拦下；试立 §一 `#558` 登记该缺口，又被**机制类可动 WIP 29/22**（协议〇.9 措施 C）阻断＋编号未预留，该行已在持锁期内原样撤回、`558` 作废不复用。⇒ **每次人工豁免属「人守被反复违反」那一族，一行 ignore 即机制化。**
...
- 🔴 **退出码只认被执行进程自己那一层**（`OP-0819-F` 2026-08-19 冒烟实证：`cmd /c "… & echo EXITCODE=%ERRORLEVEL%"` 读到 `0`、真值是 `2`；2026-09-09 由 `#440` eval-5 靶点定夺补回，此前系 `CLAUDE.md` 瘦身时蒸发、非退休）：`%VAR%` 在 cmd **解析期**即被展开——整行送进 cmd 之前那个值已经定死，读到的是**执行前**的旧值，不是被调进程的退出码。**写法**：被调命令单独落一层（独立 `.ps1`／调用后立刻读 `$LASTEXITCODE`），或在 cmd 内用 `!VAR!` ＋ `setlocal enabledelayedexpansion`；**管道之后读到的是管道末端的退出码，不是它的**。**判据一句：这个退出码是谁那一层产生的？** 同族＝上一条「工具静默回退」——错法不报错，返回一个看着很正常的 `0`。
...
### Q-2 · `.claude/rules/环境保障线.md`（瘦身前，7,885 B）
...
### Q-3 · `.claude/rules/队列与落库.md`（瘦身前，7,755 B）
...
## 队列工具在哪跑、怎么连跑（2026-09-19 `OP-0919-K` 实测补立）

🔴 **凡触碰 git 的队列／锁工具，一律在 Windows 侧跑**（`Windows-MCP PowerShell` ＋ `C:\Dev\zhuopin-ai`），不从 Cowork 挂载侧跑。**实测同一条 `git status --porcelain`：挂载侧 33.9 秒，Windows 侧 0.6 秒，56 倍**；而编辑锁每次 `acquire`／`release` 都要跑它（⑹ 登记完整性）。另一条更硬的理由：挂载侧 `rm` 被沙箱拒（`Operation not permitted`），任何「用文件当锁」的工具从那边跑，`release` 一旦被拒就清不掉锁，同一会话第二次调用必 fail-closed。**只读的 `工具-队列查询.py` 不碰 git，两侧都行（实测 0.2 秒）。**

🔴 **多行回写不要连发 `commit-edit`。** `commit-edit` ＝ acquire→edit→release 三步，而 release 会因「脏文件不属于任何 §二 批次」被拒**且锁保持占用** ⇒ 第 2 行起必撞「占用中」。正确形态是一次性：`acquire` → 多次 `edit-row --who` → `append-row --section 二` 登记批次 → 一次 `release`（脏文件若含非本批产物，用 `release --waiver '登记豁免：<路径>…（作者 <OP 号>，到期 MM-DD）'` 逐个点名）。状态格超 4096 B 先走 `工具-队列行K2外置.py --apply` 再 `edit-row --changes-json`，别用「行长豁免」凑数。

🔴 **开场扫池用 `--digest --actionable`**（2026-09-19 新增）：只出机器字段为 `open`／`partial` 且不以 🛑 起首的行。**实测 22.0 KB → 8.6 KB（−61%）**；隐藏行数随表头回显，机器字段解析不了的行一律保留不吞。要看全池去掉该参数即可。

🔴 **`*-main-leak.patch` 禁 `cat`**，走 `python 0-学习与工具/工具-main-leak回放.py`。判 FAIL 只需要泄漏清单里的文件名，不需要 diff 正文——实测 `cat` 单份补丁 **69.7 KB**（里面大半是队列行的完整 diff），回放器出全量逐条表 **1.8 KB**。候选修法用 `--extra '<glob>'` 追加白名单试算；白名单判据现取自 `工具-opener批处理执行v2.ps1`，解析不到即 fail-loud，**不在回放器里留第二份硬编码**。
````

### Q-4 · `.claude/rules/场景建造与合规.md`（瘦身前，5,966 B）
...
- 🔴 **新场景 `propose` 前置＝场景目录 `intent.md`（Shao Peishen 2026-09-06 定，承手册 intent 层）**：新场景（`4-数字员工/<部门>/<场景>/`）走 `/opsx:propose` 之前，同目录须有 `intent.md`＝skill `zhuopin-requirement-grill` 的收工产出（frontmatter `status: 已确认` 由 Shao Peishen 确认后写；三节固定：§一 M2 自查事实／§二 已定＝design Decisions／§三 待专员＝判例包待办，每条带口径点 ID 占位），缺则不得 propose。2026-09-03 已落包的 8 个新场景包（`sc4／sc10／sc11／fi5／fi6／fi8／fi9／fi10`）逐包补 grill，产出即 design 审材料；SC2 的 `需求grill产出-2026-08-18.md` 为历史件不追改。深化类场景不适用（grill 融合方案 G3(a)）。🛡 **机器守已生效，落点＝CI job `scene-intent-gate-lint`**（`.github/workflows/ci.yml`，跑 `python 0-学习与工具/工具-场景包intent闸lint.py --enforce`；2026-09-06 `OP-0906-S` 建成，承队列 §一 `#436` ⑶①）。🔴 **不是 openspec validate，本句 2026-09-17 更正**（原写「机器守＝openspec validate 闸（队列 §一 需求侧机器守行，CC 建成前本条人守）」，其中 openspec validate 闸与那条队列行**都不存在**）：09-06 实测 openspec 1.7.0 的 `rules.proposal` 只是喂给起草方的提示文本、`validate` 根本不读它——本仓 8 份缺 config.yaml 点名 MANDATORY 节的 `proposal.md`，`openspec validate --all --strict` 仍 169 passed；`openspec --help` 全表亦无自定义规则挂载点。**⇒ 提示句不是闸，lint 才是。** **存量 8 包（`sc4`／`sc10`／`sc11`／`fi5`／`fi6`／`fi8`／`fi9`／`fi10`）已在 lint 内豁免至 2026-10-31**，到期前须逐包补 grill。🔑 **本次更正的教训值得留**：这条自陈把人指向了一个不存在的落点，而真实落点在第三处——**无载体只是空的，指错载体是错的**，后者会让人重复建造（2026-09-17 `OP-0911-H` 险些据此派一轮 CC 去重建一个已在 CI 上跑的闸）。
...
### Q-5 · `.claude/rules/跟进信与专员.md`（瘦身前，5,231 B）
...
- 🔴 **读到规则 ≠ 执行规则**：起草完成后跑 `python 0-学习与工具/工具-称谓核对.py <信件.md> <信件.docx>`（退出码 0 才算过；判据见 `6-人才与组织/部门AI专员跟进/跟进机制-判据版.md` §四.2）；机器守＝编辑锁 `release` 触碰信件时强制跑同一判据（`#566`）＋ CC 侧 PostToolUse 哨兵 `sentinel-pronoun.ps1`。

- 🔴 **称谓守卫的两处未挂点（`#566` 关行前迁出的下游候选，2026-09-13 `OP-0913-E` 迁入；**触发式、不是待办**）**：① 改 `approve_followup_letter.py`（批准脚本＝发送侧最后一道关卡）时**须同批把 `gender_pronoun_findings` 同判据前置挂上**——它是发送链路上目前唯一没挂的一环；② H4 哨兵 `sentinel-pronoun.ps1` 与 `0-学习与工具/工具-称谓核对.py` 仍是**两份并行判据**（哨兵运行时解析名录散文、本工具走常量），改动任一侧须同步另一侧，合并属另一棒。

## 二、跟进信纪律
- **归集** `6-人才与组织/部门AI专员跟进/`，命名 `部门-姓名-跟进-YYYY-MM-DD-主要事项.md`，每封含**做什么／怎么做／什么时候交**。🔴 **登记只用 CLI**：发一封用 `python 0-学习与工具/工具-跟进信README登记.py append` 登记、改状态用 `set-status`，**不手工 Edit 主表**（编号 `部门#N` 由 CLI 按部门计数器自动取，未发出不占号、已发旧信不改名）；**判状态只用** `工具-跟进信README查询.py --digest` 或 `工具-跟进闸查询.py --to <收信人>`，**不 Read／grep 主表全文**（CC 侧门禁 `hooks-pretooluse-queue-read-guard.ps1` 机器守，Cowork 侧人守；主表行长口径＝主要事项 ≤600 B／发送状态 ≤4 KB，超限外置 `跟进信行日志/<编号>.md`，2026-09-13 起 release 阻断）。
...
### Q-6 · `.claude/rules/文档与全景治理.md`（瘦身前，4,243 B，续棒 `OP-0921-F` 补archive）

> **成因**：第一棒瘦身五份时本份因「本来就短」未动；续棒因 rules 合计仍超 `CLAUDE_MD_RULES_TOTAL_BYTE_CAP`（30,720 B）而一并瘦身，补齐五份之外的第六份存档。
...
- 🔴 **排期＝不晚于，不是开工日（Shao Peishen 2026-09-08 定）**：权威排期表的月份是承诺下限；**只要前置总表无阻碍项（或阻碍已解除），任一场景可提前立行、提前 grill、提前建造**，不必等排期月、不必先改排期表；**业务行 `[D:业]` 不受协议〇.9 机制类 WIP 上限约束，能开尽开**——真正的节拍只剩 grill（他在场 1 h／场景）、专员串行闸与看护批并行 4。提前完成的场景由全景线在下一次重组循环把排期表改成实际月并登 §0.2（不追改历史）。判据一句：**排期只回答「最晚何时」，不回答「能否现在」；「能否现在」只看前置总表。**
...
## 附录 R · zhuopin-lane-watch SKILL.md 瘦身前全文存档 —— 2026-09-21（队列 §一 `#629`，本次任务）

> **成因**：`SKILL.md` 现取约 45,119 B，超"开场必读四件合计 33,336 B"，且是泳道看护每棒固定要读的正本；瘦身口径同附录 M/N——只删成因/实证/历史叙事，判据/动作/命令一条不删。
...
**① 回归闸 narrow 到受影响的测试类，不为一个类跑整份测试文件。** `工具-泳道分支合入.ps1 -Tests` 收的是**逗号分隔的 pytest 目标**，所以用 **node-id**：`-Tests "<测试文件>::<类名>"`。🔴 **不能写 `-k`**——脚本 L116 把整串当**一个** argv 递给 pytest，`"<文件> -k <类>"` 会被当成路径，随后 `Get-PytestFailedTests` 拿到 ErrorRecord 而抄 `.Trim()`，整个合入 `exit=9` 中断（`OP-0919-A` 实擞一次）。多个类就逗号串起来。六关一关不减（④ 仍是「与纯 master 比失败集合」，只是两侧用同一个 narrow 口径）。🔑 **实测代价差**：`#507` 改的只是 `ManifestSkipEscalationTests` 一个类，跑整份 `test_工具-落库sweep.py` 是 **29 分钟 × 2 侧**，narrow 到该类是 **2.5 分钟 × 2 侧**，差 12 倍；而两者能答的问题（本改动有没有打坏东西）是同一个。改动跨多个类时就列多个 `-k`，**不是回到全量**。

**② 长任务写哨兵文件，只看哨兵，不逐分钟 tail 日志。** 超过几分钟的活一律 `Start-Process ... -RedirectStandardOutput <log>`，**并让它跑完自己落一个哨兵文件**（如 `<log>.done`，内容写退出码）；看护侧只探哨兵在不在，不在就**大间隔**再来（定时回检／`send_later`，不是每分钟戳一次）。🔴 **成因**：桥接单次上限 60 秒，而每戳一次都要重发整段上下文——`OP-0919-A` 实擞：一轮 ff 等待戳了 25+ 次，带回的信息只有一行日志，开销全花在「等」不在「干」。🔴 **重跑同一日志路径前先删旧哨兵**（2026-09-20 队列 §一 `#627` 补）——`Remove-Item ($log + '.done') -ErrorAction SilentlyContinue`；判「完成」看 `.done` 的 **mtime 是否晚于本轮起跑时刻**，不能只看文件在不在，否则会把上一轮的旧哨兵当成本轮结果误判（实撞：看护者复用同日志路径重跑 ff，旧 `.done` 未删，被当成本轮结果误报 `exit=9`）。
...
- v2.4.1（2026-09-19 `OP-0919-A` 出件：新增「ff 与长任务纪律」节——回归闸 narrow 到受影响测试类、长任务写哨兵只看哨兵。Shao Peishen 2026-09-19 定，**所有项目遵守**；取证见本日 `#507` ff 实测 29min×2 vs 2.5min×2。判据、阈值、六关本体均未改。）
...
- v2.2.1（2026-09-10 `OP-0910-M` 出件：步骤 4 收工心跳样例补 `--batch <批次>`，并写明缺失时的非静默降级口径（工具不猜批次、当场提示、`summary` 单列「归属未知」）。队列 §一 `#544`（源 §四 `#195`，`#536` 的写侧残余）；仅补样例与口径说明，不改判据、不改看门狗阈值。小号原为 v2.3，2026-09-19 ff 时重编：本文件侧的那段文字已被 master 上更完整的 `#565`／`#536` 写法覆盖（同一口径、措辞更宽），故文档段取 master 侧；本版实际落地的是 **写侧代码与单测**（`工具-泳道看护状态机.py` +47／单测 +101），master 此前只有读侧。）
