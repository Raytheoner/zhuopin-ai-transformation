...
## 附录 Q · `.claude/rules/*.md` 五份瘦身前全文存档 —— 2026-09-21（队列 §一 `#629`「读文件瘦身」）

> **成因**：`.claude/rules/*.md` 六份合计现取 **43,419 B**，超机制阈值 `工具-落库sweep.py::CLAUDE_MD_RULES_TOTAL_BYTE_CAP`（30,720 B）**约 12,700 B**；其中 `两桌同步与取证.md` 单份 **12,339 B**，超单份阈值 `CLAUDE_MD_RULES_BYTE_CAP`（8,192 B）**约 4,150 B**。瘦身口径同附录 M：六份文件头本已自述「只写触发→动作→判据／指针；成因／实证一律在 CHANGELOG」，本次是让正文真正遵守这条——逐条只删**成因／谁定何时定／原话引用／具体事故细节／实测数字叙事**，**判据／触发／动作一条不删**。落盘前用关键短语核对表逐条核对，缺一条不允许写盘。因总量缺口远大于单份超额，本次除主力目标 `两桌同步与取证.md` 外，`环境保障线.md`／`队列与落库.md`／`场景建造与合规.md`／`跟进信与专员.md` 四份也一并瘦身；`文档与全景治理.md`（4,243 B，本来就短）不动。五份瘦身前的**当前完整原文**逐字存档如下。
