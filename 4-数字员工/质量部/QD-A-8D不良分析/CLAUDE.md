...
## 时间线
- **2026-07-04（轨 A 校准跑通）**：原始 8D 为 .pptx（原 reader 仅 docx/pdf），已打通：pptx 解析 + 段落识别加「D1.」点号 & 无前缀标题回退（files 4-7）；xlsx「8D历史库录入表」黄金加载；12 字段可信度地图（候选）+ 逐字段 diff。**5/7 可校准**；案例1/2 **OneDrive 截断**（恰好 10MB/20MB、非有效 pptx，待 Paul 强制下载重跑）。脱敏加固：邮箱泄漏堵住（原全漏）、平台误匹配 362→1-4、供应商前缀修复。**校准结果在 `results/`（gitignore/LAN）交陈忱校准会审定档位终版**。31→41 tests。3 个坑（根因验证口径矛盾/案例4-5 安全相关复核/案例4-5 分类分歧）脚本只标不判、待陈忱。
