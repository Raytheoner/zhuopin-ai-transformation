"""FI2 三单匹配一键运行 —— FeedSource → 明细分类 → 报告（+ 审计）。
...
def run(data_source: str | None = None, *, mock_dir: Path | None = None,
    ...
    grn_rows = fs.load_grn()
    invoice_rows = fs.load_invoice()

    linked, orphaned = partition_invoices(po_lines, invoice_rows)
    lines = classify_all(po_lines, grn_rows, linked)

    return build_report(
        lines, orphaned,
        data_sources={"po": fs.data_source, "grn": fs.data_source, "invoice": fs.data_source},
        evaluator=evaluator, period=period, audit=audit,
    )


def main() -> None:
    ap = argparse.ArgumentParser(description="FI2 三单匹配自动对账（MVP）")
    ...
    print(f"  PO行 {s['total']}：L3建议通过 {s['l3_suggested_pass']} / 需人工确认 {s['needs_review']}")
