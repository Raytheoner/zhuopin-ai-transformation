...
#
# ✅ **2026-09-20 追加签认：唐燕萍 `财务部#19` J1 选 (c)**——判例 7／8 互斥（同一形态
# 「凑法跨两个料号」在判例 7 写「一律不认领」、在判例 8 却「拆两条认领」）的裁定补一条
# 条件：**只在两个料号同属一张 AP 单时才拆；不同 AP 单的组合改按判例 7 办，一律不认领**
# （§四 `#196` 已闭环，取证见 `1-转型规划/0-全景路线图/拆件巡逻报告-2026-09-20.md`）。
# 🔑 **这条件在本函数的调用边界上已经是唯一可能——不是新加的判断，是把既有边界写清楚
# 并加测试锁死**：`_subset_sum_split_plan` 只吃 `ap_lines_for_ap_no`（调用方按单一
# `ap_no` 取好、见 `_IngestState._ap_lines_cache[ap_no] = connector.get_ap_lines(ap_no)`），
# 凑法涉及的每个料号必然来自这**同一张** AP 单——不存在跨 `ap_no` 凑法的代码路径，
# 故「不同 AP 单不拆」不需要新增运行时判断，只需不去打破这条调用边界。
# 端到端回归见 `tests/test_tax_export_ingest.py::test_split_does_not_leak_item_codes_across_different_ap_documents`
# （两张真实 AP 单气泡袋 `AP-2026080137`＋密封胶 `AP-2026080041` 同批摄取，互不掺入）。
...
def resolve_item_code_split(
    ...
    *,
    ...
    唐燕萍 2026-09-04 判例 8 勾 ✅。触发条件刻意收得极窄，逐条都是「不猜」的具体形态：
      ...
      ⑸ 🔴 **跨的这几个料号须同属一张 AP 单**（唐燕萍 2026-09-20 `财务部#19` J1
         追加条件；不同 AP 单的组合按判例 7 办，一律不认领）——本条不靠一处新判断，
         靠调用边界天然满足：`ap_lines_for_ap_no` 本就是调用方按**单一** `ap_no`
         取好的行项目（见本函数签名），凑法里的每个料号必然来自这同一张单。
