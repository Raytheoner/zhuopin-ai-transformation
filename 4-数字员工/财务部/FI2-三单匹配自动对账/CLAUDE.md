...
- **2026-09-07 `#423` 改判到判例 3 ＋ `#424` 摄取层认领口径与拆行——已部署 `.51`+行为冒烟通过**
  （opener `OP-0907-N`，worktree `op0907n-fi2-verdict-3`，commit `0b32c65`；口径依据＝唐燕萍
  2026-09-04 `财务部#16` 回件逐格勾选，2026-09-07 复核纠正我方 09-04 的拆件误判）。
  **改了什么**：`_INVOICE_SCOPE` → `ap_range`（判例 3 ✅）；孤立发票不再铺进三单核对明细表、
  改由新的「未匹配成功的发票池」承接（判例 4 ❌ 的正解＝**不在明细表露面但留得住查得到**，
  绝非丢掉）；`_ITEM_MATCH_STRATEGY` → ⒜金额→⒝单料号→⒞组合求和依次回落（判例 5/6/7 ✅）；
  新增 `_ALLOW_INVOICE_ROW_SPLIT` 拆行路径（判例 8 ✅，尾差并入最后一条）。
  **部署**：`sync-to-server.ps1` 推送，`Fi2WebServer` 新进程存活（`NEW_PID=5576`
  `CREATED=2026-09-07 10:02:49`）；部署件 SHA256 与本地 CRLF 归一后逐字节一致
  （`webapp.py` `93055ff2…`／`tax_export_ingest.py` `5c7798bc…`）。
  **冒烟**：`/api/ping` 200；`/` 302 到门禁登录页（预期，门禁生效中）；
  新增 `scripts/probe_423_verdict3_smoke.py` 在 `.51` 本机驱动面板同一条管线
  （`webapp._run_with_detail`），筛 `AP-2026080041` 实测 **本次共 1 项料品／0 自动通过／
  0 微差消化／1 BLOCK／孤立发票 0**，未匹配发票池 **3391 行全部留存可检索** ⇒ 冒烟通过。
  🔑 **对照**：改判前同一张单面板显示「共 3390 项料品、BLOCK 3390」而引擎实际只判了 1 项
  （差三个数量级，队列 #423／#131）。
  **真实单据端到端复现**（live U9C 只读）：`AP-2026080041` 密封胶 20支vs6200 → `R02D.0001` ✅；
  `AP-2026080137` 气泡袋 7000＝4000+3000 → `R02E.0016` ✅；6000＝1000+5000 跨料号 → 拆两条
  `J02E.0024`/1000/176.99/23.01 ＋ `R02E.0024`/5000/884.96/115.04，**与真实 AP 行的
  `NonTaxAmtTC`/`TaxAmtTC` 分毫不差**、相加等于原发票行。
  **测试**：249 passed / 9 skipped（原 230+9），零回归；真实集成 7 passed（live U9C）；
  新增用例实测「在旧默认值下变红」6/7。回滚 SOP：`docs/回滚SOP-423-424-判例3与认领口径-2026-09-07.md`
  （三个独立常量开关；判例 4 的落地是代码结构、不在「改一个常量」范围内）。
  🔴 **唯一未闭合项——`#424` 已部署但「尚未生效在她眼前」**：`.51` 的
  `data/tax_export/invoice.csv` 仍是 **2026-08-28 10:30 用旧口径产出的 3391 行**，
  `Fi2TaxExportDailyScan` **自 2026-08-28 起 Disabled**（恢复权在 Shao Peishen，见队列 `#418`⑸②）。
  实测她那两张单在现网 `invoice.csv` 里**各 0 行** ⇒ **不重跑摄取，判例 5/6/7/8 一行都不会
  体现在面板上**。且 ledger 里排队待重试的只有 **86 行**（`#418` 的重试队列是后加的，
  更早被丢的 1,813 行从未入队）⇒ 要捞回实测可解的那 645 行须**全量重建**
  （`scripts/rebuild_invoice_csv.py` 或重跑 30 份导出），那是另一个待拍板的动作。
