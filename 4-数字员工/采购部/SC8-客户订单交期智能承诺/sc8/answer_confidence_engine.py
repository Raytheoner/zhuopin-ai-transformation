"""
供应商在途订单风险评估（收割自 supplychain/src/agents/supplier_tracking.py，
原 SC3「供应商在途跟踪与绩效」引擎；2026-07-06 v2.3 重排随 SC3 场景编号退役
迁入 SC8 作为内部子模块——答交可信度评分，是 SC8 交期承诺置信度未来
2→3 级化的判据来源。

本次迁移只搬运代码，尚未接入 SC8 现有承诺/置信度主流程（commitment.py /
forecast.py 不动）；接线是"随 SC8 深化"的独立后续任务。
...
# ---------------------------------------------------------------------------
# compute_dos（原 SC3 D2=A：场景本地；迁移后随宿主 sc8 继续本地存放，
# 第 2 个消费方出现时再评估提升到 zhuopin_platform/shared_tools/supply_metrics.py）
# ---------------------------------------------------------------------------
