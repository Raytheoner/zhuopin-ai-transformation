...
"""
    ...
    note = (
        ...
        '<li>✅ <b>齐料日期口径已确认并已修正</b>（队列 #344，2026-08-24 上线）：成品看板上方的'
        '齐料日期/瓶颈物料，此前只取该物料<b>最早的那笔答交日期、完全不看答交了多少</b>'
        '（连答交数量为 0 的记录也照用）；现已改为<b>按答交数量累计到覆盖需求为止的那一笔的日期</b>，'
        '与本视图、与成品卡片下方的 BOM 缺口清单<b>共用同一个累计函数</b>。'
        '口径经采购部判例批改逐条签认。<b>连带效果：成品看板上红色的行会比之前多</b>——'
        '那不是看板坏了，是此前被低估的风险被如实显示出来了。</li>'
        '</ol></div>'
    )
    return (
        '<!DOCTYPE html>\n<html lang="zh-CN"><head><meta charset="utf-8">\n'
