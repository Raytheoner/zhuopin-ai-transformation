...
- ✅ **齐料日期/瓶颈物料只按最早答交日期取值、不看答交数量 —— 已修复（2026-08-24，队列 #344，`sc8-kit-date-qty-cumulative`）**。原缺陷（2026-08-18 实测定性，姚祖怡 `F02N.0224` 举证）：`sources.load_srm_deliveries` 合并时取 `min(auth_dates)` 且 `qty_committed` 恒写 0，`forecast.estimate_material_arrivals` 的 `srm_index` 同样只留最早日期 ⇒ 答交数量为 0 的记录也会被当成到货日（真实取数 `R01I.0622`：2026-08-20 答交数量 0 被引擎采用／2027-05-20 的 10000 才是唯一有量的一笔）。**现改为按已签认的四条口径取值**（判例批改 3 条 ✅ 全签认 ＋ 口径 ⑷ 文本回件）：按答交数量累计到覆盖需求为止的那一笔的日期；一笔即够不再往后累；不够继续累计到够；答交数量为 0 等同无答交。🔑 **结构性收益比修好这一处更值得记**：`_cumulative_confirmed_batches` 已由 `baoguan.py` **下沉到 `forecast.py`**，齐料日／BOM 缺口清单／物料看板三处**共用同一个函数对象**——「下面清单对、上面汇总数不对」这一族在齐料日这个落点上从此不可能复发，而不是靠「记得同步改三处」。**与 #266 分属两层**（那条是 BOM 展开深度，这条是答交数量匹配）。
  🔴 **两处未闭合，都已登记、都无默认**：⑴ **「答了但累计仍不够」**（如需求 15000 只答了 8000）签认口径未覆盖，现按保守外推处理（判无答交 ＋ 取 `max(启发式估算日, 最晚正数答交日)`），**待姚祖怡以判例确认**；⑵ ~~**规则 1 实测未覆盖**~~ **已建造，待真实核验与签认（2026-08-25，队列 #401）**——见下条。两项详见 `docs/queue_344_kit_date修复前后对照-2026-08-24.md`。
- 🟡 **无答交起算点「规则 1」已建造、默认关闭、待真实核验与姚祖怡签认（2026-08-25，队列 #401，`sc8-kit-date-rule1-start`）**。判据 ＝ 他 2026-08-18 书面签认的规则 1（出货日不在三个月内 ⇒ 起算点 ＝ 出货日往前推 3 个自然月那月 **20 日**，再 +90），排期 ＝ Shao Peishen 2026-08-25 拍板 (a)（§四 #111）。**实现**：`forecast.no_feedback_start_date()` 是起算点的**唯一**决策点（`ship_within_horizon()` 为分界判据），`baoguan` 只传值不判分支；`estimate_material_arrivals` 新增可选入参 `heuristic_base_date`，缺省 `None` ⇒ 逐字节沿用 `demand_date` 起算。**开关 `SC8_KIT_DATE_RULE1` 默认 OFF，与 `SC8_NET_INVENTORY` 同级纪律**——翻 ON 会改保供四色，🔴 **硬前置 ＝ 真实前后对照表出件 ＋ 姚祖怡复核签认**（随采购部#19，§一 #402）。
  🔴 **一条不得被读反的实测结论**：把开关强制 `on` 跑 SC8 全量，**结果与 OFF 完全相同（502 passed 两侧一致）**——零漂移是结构性的（`pipeline.py` 与黄金基准走不进新分支），但这同时说明 **mock 语料里没有任何一行的出货日落在其 `today` 夹具三个月之外，故 mock 全量回归对规则 1 没有端到端覆盖力**。规则 1 的正确性目前只由新增的 23 条单测承担，**「全量绿」≠「已验证」**。
  🔴 **三步 LAN 留步**（本机 off-LAN，均未做）：① SRM 真实数据核验＋真实对照表（`scripts/compare_kit_date_rule1.py`，回网段一条命令，自带三条不变式自检）；② 黄金基准真实重跑（本次已用冻结件跑过、零漂移）；③ `.51:8091` 部署 ＋ 翻开关。**部署与翻开关刻意分成两步**：部署完看板一个数字都不会变，判据风险与部署风险不必同时承担。清单与复跑路径见 `docs/queue_401_规则1建造与LAN留步-2026-08-25.md`。
  ⚠️ **本次刻意没改规则 2**：出货日在未来但仍在三个月内的 24 行，现行起算点是出货日、他的规则 2 逐字是此时此刻，现行更晚更保守。#111 拍板 (a) 的标的是规则 1；顺手一起改会让一次上线带两个自变量——那正是 #344 拒绝顺手改规则 1 的理由。已在 docstring 与单测各钉一次，将来要改的人会先被那条单测拦一下。
