...
| **当前** | **SC8 保供看板 LAN 可用（内部），`SC8_NET_INVENTORY=on` 已生效**（净额现货抵扣，见 2026-07-14 行，姚祖怡已抽验确认线上数据正确）；对客外发全程关闭；`sc8-real-data-cutover` 变更包待 Paul 审核偏差数据后继续。待办 #10：加登录/Token 鉴权再开外网（真实客户名红线）；🔴 待办：`sync-to-server.ps1` 重启可靠性修复（2026-07-14 首次部署未真正重启的那次事故，本次翻 ON 重启已正常，但脚本本身仍未补代码修复，留作待办）；🟡 待办：完整系统性 LAN 真实回归报告是否仍需单独出具（跨桌任务队列 #24，21 张抽验已过，可能可作等效验收）。 |
