...
def test_波动标记不驱动任何推送():
    """🔴 姚祖怡限定「仅作为工作量参考使用」⇒ 波动标记**永远不得**成为发不发的依据。

    ⚠️ **本条在 2026-08-25 取消确认发布前置（§四 #89）之后反而更要紧了**：
    过去推送要先过人工确认门，即便推送层误读了波动标记，也还有一个人挡在前面；
    现在周五 20:00 到点即自动发，**这条断言是「波动标记不触发推送」仅剩的守卫**。

    做法仍是从源码层面反证：推送层根本不认识波动标记这个概念。
    """
    import inspect
