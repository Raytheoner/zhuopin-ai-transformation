...
> **🔴 memory 层已停用（2026-08-21 OP-0821-B 判死，2026-08-28 勘误归因）**：**完全忽略 memory 层——不读、不写、不引用**；容器内文件不删不并。**跨会话纪律的权威载体只有本 CLAUDE.md 与两份队列**，🔴 **不得新建第四份载体**（含 chat 个人层 `/areas/zhuopin-*`，两桌都读不到）。⇒ 判死实测与归因勘误（MSIX 虚拟化致写侧落真路径、注入侧读包容器）见 `1-转型规划/0-全景路线图/构建环境自动纠错与上下文治理-方案-2026-08-28.md` §一 ＋ 同目录 `memory索引收割对账-2026-08-21.md`。
