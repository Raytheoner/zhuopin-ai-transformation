...
> **🔴 memory 层已收割并停用（2026-08-21，OP-0821-B）**：三种会话类型逐一实测**均无写入口**，本项目 42 件处于「**只被注入、无任何会话可写**」（`Write` 报 success 不落盘，fail-silent）；索引层已收割完毕（8 条落入本文件 §5 与各专题载体，20 条本已覆盖，13 条作废）⇒ **此后完全忽略 memory 层：不读、不写、不引用**。存档对账见 `1-转型规划/0-全景路线图/memory索引收割对账-2026-08-21.md`，判死实测见同目录 `memory与上下文预算治理-审核与方案-2026-08-21.md`。🔴 **根因勘误（2026-08-28，Shao Peishen 拍板 3a）**：「无写入口、fail-silent」的**归因**有误——实为 Claude Desktop 系 MSIX 打包应用（`Get-AppxPackage` 实测坐实），AppData 虚拟化致**写侧落真路径、注入侧读包容器**：写进去了，只是永远从另一个目录被查；现象与「停用」结论**均不变**，容器内文件不删不并。勘误正本见同目录 `构建环境自动纠错与上下文治理-方案-2026-08-28.md` §一。🔑 **教训**：**凡「已复核／已回读／已逐字比对」类自陈，必须同时写出用什么核的（工具名／命令／哈希值）；只有动作、没有手段的验证声明，等于没有验证。** 🔴 **不得新建第四份载体**（含 chat 个人层 `/areas/zhuopin-*`——Cowork 与 CC 都读不到）。**跨会话纪律的权威载体只有本 CLAUDE.md 与队列。**
